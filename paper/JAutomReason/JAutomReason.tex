...
\newcommand{\agdahref}{81c5ef38362e90ff940727c897acbe35f427bdad}
...
\newcommand{\otz}{\ot_{\mathsf{0}}}
\newcommand{\Rarrz}{\Rarr_{\mathsf{0}}}
\newcommand{\Larrz}{\Larr_{\mathsf{0}}}
\newcommand{\oto}{\ot_{\mathsf{1}}}
\newcommand{\Rarro}{\Rarr_{\mathsf{1}}}
\newcommand{\Larro}{\Larr_{\mathsf{1}}}
...
\newcommand{\Gd}{\delta}
\newcommand{\Gl}{\lambda}
\newcommand{\sound}{\mf{sound}}
\newcommand{\complt}{\mf{complt}}
\newcommand{\piR}{\pi_{\mf{R}}}
\newcommand{\piRm}{\pi^{-1}_{\mf{R}}}
\newcommand{\piL}{\pi_{\mf{L}}}
\newcommand{\piLm}{\pi^{-1}_{\mf{L}}}
...
\newcommand{\MBCC}{\mf{MBiCCat}}
\newcommand{\FMBCC}{\mf{FMBiCC}(\mf{At})}
...
\newcommand{\Hom}{\mf{Hom}}
\newcommand{\Homs}[2]{\mf{Hom} ~ #1 ~ #2}
\newcommand{\HomC}[3]{\mf{Hom}^\mb{#1} ~ #2 ~ #3}
\newcommand{\Eq}[2]{\mf{Eq} ~ #1 ~ #2}
\newcommand{\EqC}[3]{\mf{Eq}^\mb{#1} ~ #2 ~ #3}
\newcommand{\CompC}[3]{\mf{Comp}^\mb{#1} ~ #2 ~ #3}
\newcommand{\IdC}[1]{\mf{Id}^\mb{#1}}
\newcommand{\refl}{\mf{refl}}
...
\let\oldcase\case
\let\oldendcase\endcase
\renewenvironment{case}
{\oldcase\normalsize}
{\oldendcase}
...
In the sequent calculus for this logic, antecedents are binary trees where formulae are stored as leaves. The structure of the antecedents creates subtleties when proving logical properties since in many cases one needs to analyze equalities of different presentations of the same tree. 
We formally characterize all the cases of how two presentations of a tree can be equal in Agda.
The resulting technical lemma helps us to exhaust all possible cases when proving several metalogical properties of the calculus, including cut admissibility and the Maehara interpolation property which implies Craig interpolation.
Moreover, both the cut rule and the interpolation procedure are well-defined wrt.\ an equivalence relation of derivations.
We additionally prove the proof-relevant versions of ($i$) soundness and completeness with respect to a Hilbert-style axiomatic calculus and ($ii$) Maehara interpolation, i.e.\ the interpolation procedure as a right inverse of the admissible cut rule. }
...
\keywords{nonassociative Lambek calculus, Agda, cut admissibility, Maehara interpolation, proof-relevant interpolation, categorical semantics}
...
In the sequent calculus of associative Lambek calculus (\Lam), sequents are of the form $\GG \vd C$, where $\GG$ is a \emph{list} of formulae and $C$ is a single formula. In contrast, sequents in the nonassociative Lambek calculus (\NL) are of the form $T \vd C$ where $T$ is a \emph{binary tree} in which formulae are stored in the leaves.
This structural difference makes the proofs of logical properties for \NL\ trickier than \Lam.
For example, when proving cut admissibility, in certain cases, one must determine whether the cut formula coincides with the principal formula of the conclusion. For \Lam, the flat antecedents make the situation clear: there are three possibilities, either ($i$) the cut formula is in the left context relative to the principal formula, ($ii$) in the right context, or ($iii$) it coincides with the principal formula.
...
We can intuitively understand what occurs in the proof of cut admissibility, since it is not too difficult to compute all possible cases. However, what about more complex properties?  For example, consider proving that the admissible cut rule is associative, i.e. the two ways of cutting three sequents of the form $U \vdash D$, $\mc{C}[D] \vdash E$ and $\mc{C}'[E] \vdash C$ produce equal derivations. 
% Because of the large number of cases arising from the induction on the shape of derivations and, mostly, from the analysis of equalities of trees discussed in the previous paragraph, establishing the associativity of cut becomes very challenging to perform with pen and paper, as it is hard to keep track of all the possible cases to consider.
The large number of cases arise from the induction on the shape of derivations and the analysis of equalities of trees discussed in the previous paragraph.
These cases make the proof associativity of cut becomes very challenging to perform with pen and paper, as it is hard to keep track of all the possible cases to consider.
...
Next, we present a Hilbert-style axiomatic calculus for \NL.
This is directly inspired by the categorical models of \NL, which are magmatic biclosed categories \cite{Wijnholds2017}.
These are similar to monoidal biclosed categories, but are not required to include structural morphisms of associativity and unitality (usually called associators and unitors). Alternatively, magmatic biclosed categories can be seen as a categorical generalization of residuated groupoids.
The first axiomatic calculus for \NL\ was proposed by Lambek \cite{lambek1988} for linguistic analysis.
Do{\v s}en \cite{Dosen1988} and Moortgat and Oehrle \cite{moot:categorial:2012}
%\footnote{\cheng{The book by Moot and Retore mentioned this paper but I can't find it. I am not sure if we should cite the original paper or their book.}}
also proposed different axiomatic formulations with applications in soundness and completeness wrt.\ ternary relational models and complexity of \NL, respectively.
We formalize the axiomatic calculus and the definition of magmatic biclosed categories, then show that the axiomatic calculus with its own equivalence relation on of derivations $\doteq$ generates the free magmatic biclosed category.
Moreover, we show that the axiomatic and sequent calculi are equivalent wrt.\ proofs, i.e.\ there exists a bijection between the derivations of the two calculi that preserves the equivalences of derivations.
...
%i.e. if we interpolate on two equivalent derivations, then their interpolants are identical and the corresponding derivations are pairwise equivalent.

%In this paper, we formalize \NL\ and prove properties in Agda: $(i)$ cut-elimination, $(ii)$ Maehara interpolation, $(iii)$ proof-relevant interpolation, and $(iv)$ \linebreak well-definedness of the Maehara interpolation procedure wrt.\ equivalence of derivations.
This work makes the following contributions: $(i)$ we formalize a technical lemma for dealing with equalities of the form $\mc{C}[U] = \mc{C}'[U']$; $(ii)$ we employ the lemma to formally define effective procedures for cut admissibility and Maehara (and consequently Craig) interpolation; $(iii)$ we prove proof-relevant versions of the latter, by introducing an equivalence of derivations $\circeq$ and formally establishing many properties of the defined procedures: cut is associative, commutative and unital; cut and interpolation are well-defined wrt.\ $\circeq$; interpolation is a right inverse of cut; ($iv$) we formally prove that the axiomatic calculus with the equivalence relation on of derivations $\doteq$ is equivalent to the sequent calculus wrt.\ proofs and also generates the free magmatic biclosed category. The contribution $(iii)$ is completely novel in the study of the metatheory of \NL, not only its mechanization in a proof assistant.
...
This paper is an extended version of \cite{VW2025Tableaux}. The whole Sections \ref{sec:cat:sem} and \ref{sec:axiomatic} on categorical semantics and a Hilbert-style axiomatic calculus for \NL\ are new.
They include: $(i)$ a description of magmatic biclosed categories, which are the categorical models of \NL, $(ii)$ a proof that the sequent calculus and the axiomatic calculus are equivalent, which corresponds to a proof-relevant version of soundness and completeness theorems, as well as $(iii)$ a discussion on how these constructions have been formalized in Agda.
We have also added an explanation for the interesting cases in the construction of the Maehara interpolation procedure $\mf{mip}$ and its well-definedness proof $\mf{mip}{\circeq}$ in Section \ref{sec:proof:rel:mip}.
...
The technical lemma is fundamental to our subsequent formalization of cut admissibility and Maehara interpolation, as well as for establishing properties of these procedures.

\subsection{Formulae, Trees, Substitutions}\label{sec:fma:tree}
...
When proving cut admissibility and Maehara interpolation, a common step is determining all possible cases arising from equalities of the form $\mc{C}_1[U_1] = \mc{C}_2[U_2]$. In our Agda formalization, these will be propositional equalities of the form $\sub{p_1}{U_1} \equiv \sub{p_2}{U_2}$, where $p_1$ and $p_2$ are paths in some trees $T_1$ and $T_2$ respectively.
...
\begin{array}{ll}
  ...
  \multicolumn{2}{l}{\quad \mathsf{constructor} ~ \containsright} \\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad \{W_1 ~ W_2 \} &: \Tree \\
  \;\; \quad q   &: \pathT{W_2} \\
  \;\; \quad eqT &: T_2 \equiv \sub{p_1}{(W_1 \cdast W_2)} \\
...
\section{A Sequent Calculus for \NL}\label{sec:calculus}
...
There are only three non-vacuous cases: $\oneeqtwo$ (where $D$ is exactly the principal formula $A \ot B$), $\oneLtwoR$ (where $D$ and $A \ot B$ are in different subtrees with $A \ot B$ on the left and $D$ on the right), and $\oneRtwoL$ (the dual case).
...
\section{Equivalence of Sequent Derivations}\label{sec:equiv:der}
...
The Agda definition of $\circeq$ includes: $(i)$ a constructor for each permutative conversion, $(ii)$ constructors for reflexivity, symmetry and transitivity of $\circeq$, and  $(iii)$ constructors evidencing the compatibility of $\circeq$ with the inference rules of the sequent calculus, i.e. that $\circeq$ is a congruence relation.
...
\section{Categorical Semantics}\label{sec:cat:sem}
In this section we present the categorical semantics of \NL, which is given in terms of magmatic biclosed categories and strict magmatic biclosed functors.
%and (strict) functors between them.

A \emph{magmatic biclosed category} \cite{Wijnholds2017} is a category $\mb{C}$ equipped with three functors
$${-}{\ot}{-} : \mb{C} \times \mb{C} \to \mb{C},
\qquad {-}{\Rarr}{-} : \mb{C}^{op} \times \mb{C} \to \mb{C},
\qquad {-}{\Larr}{-} : \mb{C} \times \mb{C}^{op} \to \mb{C}$$
forming two adjunctions
\begin{equation}\label{eq:adj}
  A \ot {-} \dashv A \Rarr {-}\quad \text{and}\quad {-} \ot A \dashv {-} \Larr A
\end{equation}
both natural in $A$.
This means that there is a family of bijective functions $(\piR)_{A,B,C}$ between the set of morphisms typed $A \ot B \to C$  and the set of morphisms typed $B \to A \Rarr C$, which is natural in each argument $A$, $B$ and $C$.
There is also an analogous family of bijections $(\piL)_{A,B,C}$ sending morphisms typed in $B \ot A \to C$ to morphisms typed in $B \to C \Larr A$.
We will often keep indexes implicit and simply write $\piR$ and $\piL$.

As explained in \cite{Wijnholds2017}, the name ``magmatic'' comes from the algebraic structure known as magma, which is a monoid without unit element that is not required to satisfy unitality and associativity properties.
So a magmatic category can be thought of as a monoidal category without unit object $I$ that is not required to have natural isomorphisms for unitors $\lambda,\rho$ and associator $\alpha$, as well as MacLane's triangle and pentagon coherence equations. The biclosed structure is added to capture the two implications of \NL.

We implement magmatic biclosed categories in Agda as a record type that contains three groups of data.
The first group defines includes the type $\Obj$ of objects, as well as the action of ${\ot}$, $\Rarr$ and $\Larr$ on objects, called ${\otz}$, ${\Rarrz}$ and ${\Larrz}$ (\agdaref{FCatUniversal.agda\#L11}):
 \[
\begin{array}{ll}
  \multicolumn{2}{l}{\record \:\: \MBCC : \Set_{\mf{1}} \:\: \where} \\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad \Obj &: \Set \\[2pt]
  \;\; \quad \_{\otz}\_ &: \Obj \to \Obj \to \Obj \\[2pt]
  \;\; \quad \_{\Rarrz}\_ &: \Obj \to \Obj \to \Obj \\[2pt]
  \;\; \quad \_{\Larrz}\_ &: \Obj \to \Obj \to \Obj
\end{array}
\]
The second group specifies the required morphisms (\agdaref{FCatUniversal.agda\#L20}).
There is a type $\Homs{A}{B}$ for each terms $A,B : \Obj$.
There is an identity morphism $\mf{Id}$ for each object, as well as a composition operation $\mf{Comp}$.
The action of ${\ot}$, $\Rarr$ and $\Larr$ on morphisms are called ${\oto}$, ${\Rarro}$ and ${\Larro}$. Below we only show the type of ${\Rarro}$.
Additionally, there is a family of functions $\piR$, with inverses $\pi^{-1}_{\mf{R}}$, which underlie the natural isomorphism evidencing the first adjunction in (\ref{eq:adj}).
There is also a family of functions $\piL$, with inverses $\pi^{-1}_{\mf{L}}$, for the second adjunction, which is omitted here.
%Here we additionally show five record fields which correspond to the ones in (\ref{eq:constructor:axiomatic:calc}):
\[
\begin{array}{ll}
  \;\; \quad \Hom &: \Obj \to \Obj \to \Set \\[2pt]
  \;\; \quad \mf{Id} &: \{B : \Obj\} \to \Homs{B}{B} \\[2pt]
  \;\; \quad \mf{Comp} &: \{D ~ B ~ C : \Obj\} \to \Homs{B}{C} \to \Homs{D}{B} \to \Homs{D}{C} \\[2pt]
  \;\; \quad \_{\Rarro}\_ &: \{A ~ B ~ C ~ D : \Obj\} \to \Homs{B}{A} \to \Homs{C}{D} \\
  \;\; \quad &\hspace*{0.5cm} \to \Homs{(A \Rarrz C)}{(B \Rarrz D)} \\[2pt]
  \;\; \quad \pi_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{(A \otz B)}{C} \to \Homs{B}{(A \Rarrz C)} \\[2pt]
  \;\; \quad \pi^{-1}_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{B}{(A \Rarrz C)} \to \Homs{(A \otz B)}{C} 
\end{array}
\]
%The parentheses are not part of the syntax. We include them here for better readability.

The third group specifies the required equalities between morphisms.
There is a type $\Eq{f}{g}$ for each terms $f,g : \Homs{A}{B}$.
There are record fields (not shown below) stating that $\mf{Eq}$ is an equivalence relation that is moreover a congruence relation wrt. operations $\mf{Comp}$, $\oto$, $\Rarro$, $\Larro$, $\pi_{\mf{R}}$, $\pi^{-1}_{\mf{R}}$, $\pi_{\mf{L}}$ and $\pi^{-1}_{\mf{L}}$.
There are record fields (not shown below) stating that $\mf{Comp}$ is associative and unital wrt. $\mf{Id}$.
There are record fields (not shown below) expressing that $\oto$, $\Rarro$ and $\Larro$ preserve $\mf{Id}$ and $\mf{Comp}$.
There are fields (shown below) stating that $\pi_{\mf{R}}$ and $\pi^{-1}_{\mf{R}}$ are eah other's inverses (\agdaref{FCatUniversal.agda\#L70}) and that the family of functions $\pi_{\mf{R}}$ is natural in each variable (\agdaref{FCatUniversal.agda\#L80}).
Analogous fields for $\pi_{\mf{L}}$ and $\pi^{-1}_{\mf{L}}$ are not shown.
\[
\begin{array}{ll}
  \;\; \quad \mf{Eq} &: \{A ~ B\} \to \Homs{A}{B} \to \Homs{A}{B} \to \Set \\[2pt]
  \;\; \quad \piR\mf{iso}_1 &: \{A ~ B ~ C : \Obj\} ~ \{f : \Homs{(A \otz B)}{C}\} \to \Eq{(\piRm (\piR ~ f))}{f}\\[2pt]
  \;\; \quad \piR\mf{iso}_2 &: \{A ~ B ~ C : \Obj \} ~ \{f : \Homs{B}{(A \Rarrz C)}\} \to \Eq{(\piR (\piRm ~ f))}{f}\\[2pt]
  \;\; \quad \piR\mf{nat}_1 &: \{A ~ B ~ A' : \Obj\} ~ \{f : \Homs{A'}{A}\}  \\
  \;\; \quad &\hspace*{0.5cm}\to \Eq{(\piR (f \oto \mf{Id}))}{(\mf{Comp} ~ (f \Rarro \mf{Id}) ~ (\piR ~ \mf{Id}))}\\[2pt]
  \;\; \quad \piR\mf{nat}_2 &: \{A ~ B ~ C ~ B' : \Obj\} ~ \{f : \Homs{(A \otz B)}{C}\} ~ \{g : \Homs{B'}{B}\} \\
  \;\; \quad &\hspace*{0.5cm}\to \Eq{(\mf{Comp} ~ (\piR ~ f) ~ g)}{(\piR ~ (\mf{Comp} ~ f ~ (\mf{Id} \oto g)))} \\[2pt]
  \;\; \quad \piR\mf{nat}_3 &: \{A ~ B ~ C ~ C' : \Obj\} ~ \{f : \Homs{C}{C'}\} ~ \{g : \Homs{(A \otz  B)}{C}\} \\
  \;\; \quad &\hspace*{0.5cm} \to \Eq{(\piR ~ (\mf{Comp} ~ f ~ g))}{(\mf{Comp} ~ (\mf{Id} \Rarro f) (\piR ~ g))}
\end{array}
\]
The implementation of categories using \emph{setoids} for homsets, i.e. using types $\Homs{A}{B}$ each equipped with a congruence relation $\mf{Eq}$, is standard in type theory, see e.g. \cite{HuC21}.


A \emph{strong magmatic biclosed functor} between magmatic biclosed categories $(\mb{C} , \ot , \Rarr , \Larr,\piR,\piL)$ and $(\mb{D} , \ot' , \Rarr' , \Larr',\piR',\piL')$ is a functor $F : \mb{C} \to \mb{D}$ equipped with three natural isomorphisms
\[
\begin{array}{c}
  m_{A,B} : FA \ot' FB \stackrel{\cong}{\to} F (A \ot B)\\
  r_{A,B} : F (A \Rarr B) \stackrel{\cong}{\to} FA \Rarr' FB \\
  l_{A,B} : F(B \Larr A) \stackrel{\cong}{\to} FB \Larr' FA
\end{array}
\]
appropriately preserving the adjunction data, i.e.\ for any $f : A \ot B \to C$ in $\mb{C}$,
\begin{equation}\label{eq:presadj}
r'_{A,C} \circ F(\piR ~ f) = \piR' (F f \circ m_{A,B}),
\qquad
l'_{B,C} \circ F(\piL ~ f) = \piL' (F f \circ m_{A,B}).
\end{equation}

Strong magmatic functors are like strong monoidal functors that are not required to interact with the unitors $\lambda,\rho$ and the associator $\alpha$, since this data is not present in a magmatic category. 
Strong magmatic biclosed functors appropriately relax the notion of strong monoidal biclosed functor.
%In the context of monoidal (closed) functors, the left to right direction of $m$ ($r$ or $l$) together with the commutative diagram of associativity constitutes the $\ot$ ($\Rarr$ or $\Larr$) part of a lax monoidal (closed) functor. The other direction is called a oplax monoidal (closed) functor.
%So a strong magmatic biclosed functor should be thought of as a strong monoidal biclosed functor without unit and properties of associativity. 
% A monoidal (closed) functor is strong if $m$ ($r$ or $l$) is a natural isomorphism.

% Strict magmatic biclosed functors are implemented in Agda in two steps.
Strong magmatic biclosed functors are implemented in Agda as a record type (\agdaref{FCatUniversal.agda\#L133}).
This record type includes fields for a functor with action on objects $\mf{F}_0$ and action on morphisms $\mf{F}_1$, natural isomorphisms $\mf{m}$, $\mf{r}$  and $\mf{l}$ and the preservation of adjunction data.
Below we only show a selection of relevant fields, the last one evidencing the first equation in (\ref{eq:presadj}).
We have decorated all projections $\Obj$, $\Hom$, $\otz$, etc. with either  $\mb{C}$ or $\mb{D}$, e.g. $\Obj^{\mb{C}}$ and $\Obj^{\mb{D}}$, in order to distinguish the data belonging to each category. In Agda, a similar result can be achieved by appropriately renaming all projections.
%Notice that each occurrence of a constructor has an additional argument specifying the category in which it lives.
\vspace*{-0.3cm}
 \[
 \scalebox{.97}{$
\begin{array}{ll}
  \multicolumn{2}{l}{\record \:\: \mf{MBiCFun} ~ (\mb{C} ~ \mb{D} : \MBCC) : \Set_{\mf{1}} \:\: \where} \\
%  \multicolumn{2}{l}{\quad \mf{open} ~ \MBCC}\\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad \mf{F}_0 &: \Obj^\mb{C} \to \Obj^\mb{D} \\[2pt]
  \;\; \quad \mf{F}_1 &: \{B ~ C : \Obj^\mb{C}\} \to \HomC{C}{B}{C} \to \HomC{D}{(\mf{F}_0 ~ B)}{(\mf{F}_0 ~ C)}\\[2pt]
  \;\; \quad \mf{m} &: \{B ~ C : \Obj^\mb{C}\} \to \HomC{D}{(\mf{F}_0 ~ B \otz^\mb{D} \mf{F}_0 ~ C)}{(\mf{F}_0 ~ (B \otz^\mb{C} C))} \\[2pt]
  \;\; \quad \mf{m}^{-1} &: \{B ~ C : \Obj^\mb{C}\} \to \HomC{D}{(\mf{F}_0 ~(B \otz^\mb{C} C))}{(\mf{F}_0 ~B \otz^\mb{D} \mf{F}_0 ~C)} \\[2pt]
%  \;\; \quad \mf{r}_\mf{R} &: \{B ~ C : \Obj^\mb{C}\} \to \EqC{D}{(\mf{F}_0 ~ B \Rarrz^\mb{D} \mf{F}_0 ~ C)}{(\mf{F}_0 ~ (B \Rarrz^\mb{C} C))} \\[2pt]
  \;\; \quad \mf{m}\mf{nat} &: \{B ~ C ~ D ~ E : \Obj^\mb{C}\} ~ \{f : \HomC{C}{B}{D}\} ~ \{\HomC{C}{C}{E}\} \\
  \;\; \quad &\quad \to \EqC{D}{(\CompC{D}{\mf{m}}{(\mf{F}_1 ~f \oto^\mb{D} \mf{F}_1 ~g)})}{(\CompC{D}{(\mf{F}_1 ~(f \oto^\mb{C} g))}{\mf{m}})}\\[2pt]
  \;\; \quad \mf{m}\mf{iso}_{1} &: \{B ~ C : \Obj^\mb{C}\} \to \EqC{D}{(\CompC{D}{\mf{m}}{\mf{m}^{-1}})}{(\IdC{D})} \\[2pt]
  \;\; \quad \mf{m}\mf{iso}_{2} &: \{B ~ C : \Obj^\mb{C}\} \to \EqC{D}{(\CompC{D}{\mf{m}^{-1}}{\mf{m}})}{(\IdC{D})} \\[2pt]
  \;\; \quad \mf{pres}\piR &: \{A ~ B ~ C : \Obj^\mb{C}\} ~ \{f : \HomC{C}{(A \otz^\mb{C} B)}{C}\} \\
  \;\; \quad &\quad \to \EqC{D}{(\CompC{D}{\mf{r_\mf{R}}}{(\mf{F}_1 ~(\piR^\mb{C} ~ f))})}{(\piR^ \mb{D} ~ (\CompC{D}{(\mf{F}_1 ~f)}{\mf{m}}))}
\end{array}
$}
\]

% %%previous version
% Let $(\mb{C} , \ot , \Rarr , \Larr)$ and $(\mb{D} , \ot' , \Rarr' , \Larr')$ be magmatic biclosed categories. A \emph{strict magmatic biclosed functor} $F : \mb{C} \to \mb{D}$ is a functor satisfying the equations
% $$F (A \ot B) = FA \ot' FB,
% \quad F (A \Rarr B) = FA \Rarr' FB,
% \quad F(B \Larr A) = FB \Larr' FA.$$

% Strict magmatic biclosed functors are implemented in Agda in two steps.
% First, we define a record type of strong magmatic biclosed functor (\agdaref{FCatUniversal.agda\#L133}).
% This record type contains the data of a functor, natural isomorphisms $m : F A \ot' F B \cong F (A \ot B)$, $r : F (A \Rarr B) \cong F A \Rarr' F B$, $l : F (B \Larr A) \cong F B \Larr' F A$ and preservation of adjunctions.
% In the context of monoidal (closed) functors, the left to right direction of $m$ ($r$ or $l$) together with the commutative diagram of associativity constitutes the $\ot$ ($\Rarr$ or $\Larr$) part of a lax monoidal (closed) functor. The other direction is called a oplax monoidal (closed) functor. A monoidal (closed) functor is strong if $m$ ($r$ or $l$) is a natural isomorphism.
% So a strong magmatic biclosed functor should be thought of as a strong monoidal biclosed functor without unit and associativity.

A strong magmatic biclosed functors is \emph{strict} when the three families of morphisms $m$, $r$, and $l$ are all identities.
In Agda we introduce a record type $\mf{IsStrict}$ (\agdaref{FCatUniversal.agda\#L206}) taking parameter arguments $\mb{C},\mb{D} : \MBCC$ and $F : \mf{MBiCFun} ~ \mb{C} ~ \mb{D}$, so we think of it as a predicate on the strong magmatic functor $F$.
Asking for $m$ to be equal to the identity morphism is achieved in Agda by requiring a field corresponding to a family of \emph{propositional} equalities of objects:
\[
\mf{mEq} : \{B,C : \Obj^{\mb{C}}\} \to \mf{F}_0 ~ B \otz^\mb{D} \mf{F}_0 ~ C \equiv \mf{F}_0 ~ (B \otz^\mb{C} C)
\]
and a field stating that $\mf{m}$ is equal (in the sense of $\mf{Eq}^{\mb{D}}$-related) to the identity morphism, where the codomain of the latter is appropriately corrected using the previous equality $\mf{mEq}$, in order to match the codomain of $\mf{m}$:
\[
\mf{mId} : \{B,C : \Obj^{\mb{C}}\} \to \EqC{\mb{D}}{\mf{m}}{(\subst ~(\lambda x \to \HomC{D}{\mf{F}_0 B \oto \mf{F}_0 C}{x})~ \mf{mEq} ~(\IdC{\mb{D}}))}
\]


% We then declare a record type of strict magmatic biclosed functors (\agdaref{FCatUniversal.agda\#L206}) which states that a strong magmatic biclosed functor is strict if $m$, $r$, and $l$ are actual identities rather than natural isomorphisms.

\section{A Hilbert-Style Axiomatic Calculus for \NL}\label{sec:axiomatic}

The main difference between the sequent and (Hilbert-style) axiomatic calculi lies in the shape of sequents. In the sequent calculus antecedents are list of formulae, while in the axiomatic calculus they consist of a single formula.
We write $A \lra B$ for sequents in the axiomatic calculus, whose derivations are inductively generated by the following rules:
\begin{equation}\label{eq:axiomatic:calculus}
  % \[
\begin{array}{c}
    \infer[\mf{id}]{A \lra A}{}
    \quad
    \infer[\mathsf{comp}]{A \lra C}{
    A \lra B
    &
    B \lra C
    }
    \\[5pt]
    \infer[\ot]{A \ot B \lra C \ot D}{
    A \lra C
    &
    B \lra D
    }
    \quad
    \infer[\Rarr]{A \Rarr B \lra C \Rarr D}{
    C \lra A
    &
    B \lra D
    }
    \quad
    % \infer[\otr]{A \otr B \lra C \otr D}{
    %   A \lra C
    %   &
    %   B \lra D
    % }
    % \quad
    \infer[\Larr]{B \Larr A \lra D \Larr C}{
    C \lra A
    &
    B \lra D
    }
    \\[5pt]
    \infer=[\pi_{\Rarr}]{A \lra B \Rarr C}{A \ot B \lra C}
    \quad
    \infer=[\pi_{\Larr}]{A \lra B \Larr C}{A \ot C \lra B}
  \end{array}
% \]
\end{equation}
The double lines in rules $\pi_{\Rarr}$ and $\pi_{\Larr}$ means that the rules are invertible.
The bottom-up inference rules are denoted $\pi^{-1}_{\Rarr}$ and $\pi^{-1}_{\Larr}$, respectively.

There exist various equivalent presentations of the axiomatic calculus for \NL\ in the literature, their relationship is thoroughly analyzed in \cite[Ch.~4]{moot:categorial:2012}. 
The original proposal by Lambek \cite{lambek1988} includes an axiom of identity, a transitivity rule and four rules evidencing the adjunctions between tensor and the two implications.
Do{\v s}en's version \cite{Dosen1988} replaces the four adjunction rules by four axioms, corresponding to units and counits of the adjunctions, plus three monotonicity rules, one for each logical connective.
Our formulation in (\ref{eq:axiomatic:calculus}) is closer to the one proposed by Moortgat and Oehrle \cite{moot:categorial:2012}.
Their version contains both the monotonicity and adjunction rules, but no transitivity rule, which they show to be admissible.
We include the transitivity rule $\comp$, a.k.a. morphism composition, as a primitive rule to make the calculus a direct instance of a magmatic biclosed category.
The presence of $\comp$ is also needed for stating the coherence equations of magmatic biclosed categories.
% By a direct characterization of the categorical models, we mean that the axiomatic calculus forms a category where we interpret objects as formulae, derivations as morphisms and composition as the rule $\comp$.

We implement the entailment relation $\lra$ in our Agda formalization as an inductive type family over pairs of formulae (\agdaref{FCat.agda\#L10}), i.e.\ $A \lra B$ is a type for $A ~ B : \Fma$.
There is one constructor for each rule in (\ref{eq:axiomatic:calculus}), which is the direct characterization of the rule in our formalization.
Here we show the constructors associated to $\id$, $\comp$, $\Rarr$, $\pi_{\Rarr}$ and $\pi^{-1}_{\Rarr}$:
\begin{equation}\label{eq:constructor:axiomatic:calc}
  \begin{array}{ll}
  \multicolumn{2}{l}{\data \:\:  \_{\lra}\_  : \Fma \to \Fma \to \Set \:\: \where} \\
  \;\; \quad \id &: \{A : \Fma \} \to A \lra A \\
  \;\; \quad \_{\circ}\_ &: \{A ~ B ~ C : \Fma \} ~(f : A \lra B) ~(g : B \lra C) \to A \lra C \\
  \;\; \quad \_{\Rarr}\_ &: \{A ~ B ~ C ~ D : \Fma \} ~(f : B \lra A) ~(g : C \lra D) \\
  \;\; \quad &\hspace*{0.5cm}\to A \Rarr C \lra B \Rarr D \\
  \;\; \quad \pi_{\Rarr} &: \{A ~ B ~ C : \Fma \} ~(f : A \ot B \lra C) \to B \lra A \Rarr C \\
  \;\; \quad \pi^{-1}_{\Rarr} &: \{A ~ B ~ C : \Fma \} ~(f : B \lra A \Rarr C) \to A \ot B \lra C
\end{array}
\end{equation}

To interpret derivations as morphisms, we define a congruence relation $\doteq$ on derivations, whose generating pairs are given in Figures \ref{fig:cat:eq},\ref{fig:eq:Rarr} and \ref{fig:eq:Larr}, which formalizes the equational theory of magmatic biclosed categories.
Figure \ref{fig:cat:eq} contains equations for left and right unitality of the identity morphism $\id$, associativity of composition $\comp$ and the the functoriality of $\ot$.
Figure \ref{fig:eq:Rarr} contains equations for functoriality of $\Rarr$ and the fact that $\pi_{\Rarr}$ is a natural isomorphism, natural in all three arguments.
Figure \ref{fig:eq:Larr} contains similar equations to those in Figure \ref{fig:eq:Rarr} but for $\Larr$ instead of $\Rarr$.

In Agda. the equivalence relation $\doteq$ is implemented as an inductive family indexed over pairs of derivations (\agdaref{FCat.agda\#L34}), which is similar to the formalization of $\circeq$ in Section \ref{sec:equiv:der}.
In particular, we need to remember to add rules for reflexivity, symmetry and transitivity of $\doteq$, as well as congruence rules for all constructors.
\begin{figure}[t]
  % \vspace{0.5\textwidth}
%\begin{landscape}
\[
...
~ \doteq ~
\proofbox{
\infer[\id]{A \ot B \lra A \ot B}{
}
}
\\[15pt]
\begin{array}{l}
  \proofbox{
\infer[\ot]{A \ot B \lra E \ot F}{
  \infer[\comp]{A \lra E}{
    \deduce{A \lra C}{f} &
    \deduce{C \lra E}{g}
  } &
...
\hspace*{4cm}\doteq ~
\proofbox{
\infer[\comp]{A \ot B \lra E \ot F}{
  \infer[\ot]{A \ot B \lra C \ot D}{
    \deduce{A \lra C}{f} &
    \deduce{B \lra D}{g}
  } &   
  \infer[\ot]{C \ot D \lra E \ot F}{
    \deduce{C \lra E}{g} &
    \deduce{D \lra F}{k}
  } 
}
...
\end{array}
\]
\caption{Unitality and composition laws of categories and functoriality of $\ot$}
\label{fig:cat:eq}
%\end{landscape}
\end{figure}
...
\begin{figure}
  \begin{displaymath}
    \arraycolsep=3pt
    \scalebox{1}{$\begin{array}{l}
      \begin{array}{c}
      \hspace*{1.3cm}
        \begin{array}{c}
        \infer[\Rarr]{A \Rarr B \lra A \Rarr B}{
            \infer[\id]{A \lra A}{}
            &
            \infer[\id]{B \lra B}{}
        }
      ...
      \quad \doteq \quad
      \begin{array}{c}
        \infer[\id]{A \Rarr B \lra A \Rarr B}{}
      \end{array}
      \end{array}
      \\[0.4cm]
      \begin{array}{c}
        \infer[\Rarr]{C \Rarr D \lra A \Rarr F}{
            \infer[\comp]{A \lra C}{
                \deduce{A \lra B}{f}
                &
                \deduce{B \lra C}{g}
      ...
      \hspace*{3cm}\doteq
      \begin{array}{c}
        \infer[\comp]{C \Rarr D \lra A \Rarr F}{
            \infer[\Rarr]{C \Rarr D \lra B \Rarr E}{
                \deduce{B \lra C}{g}
                &
                \deduce{D \lra E}{h}
            }
            &
            \infer[\Rarr]{B \Rarr E \lra A \Rarr F}{
                \deduce{A \lra B}{f}
                &
                \deduce{E \lra F}{k}
            }
      ...
      \\[0.4cm]
      \begin{array}{c}
        \infer[\pi_{\Rarr}]{B \lra A  \Rarr (A' \ot B)}{
        \infer[\ot]{A \ot B \lra A' \ot B}{
          \deduce{A \lra A'}{f}
          &
          \infer[\id]{B \lra B}{}
      ...
      \hspace*{1cm}\doteq \ 
      \begin{array}{c}
        \infer[\comp]{B \lra A  \Rarr (A' \ot B)}{
            \infer[\pi_{\Rarr}]{B \lra A'  \Rarr (A' \ot B)}{
              \infer[\id]{A' \ot B \lra A' \ot B}{}
            }
            &
            \infer[\Rarr]{A'  \Rarr (A' \ot B) \lra A  \Rarr (A' \ot B)}{
              \deduce{A \lra A'}{f}
              &
              \infer[\id]{A' \ot B \lra A' \ot B}{}
            }
      ...
      \\[0.4cm]
      \begin{array}{c}
        \infer[\comp]{B' \lra A \Rarr C}{
            \deduce{B' \lra B}{f}
            &
            \infer[\pi_{\Rarr}]{B \lra A \Rarr C}{
              \deduce{A \ot B \lra C}{g}
            }
        }
      \end{array}
      % \hspace*{4cm}
      ...
      \begin{array}{c}
        \infer[\comp]{B \lra A \Rarr C'}{
            ...
            &
            \infer[\Rarr]{A \Rarr C \lra A \Rarr C'}{
              \infer[\id]{A \lra A}{}
              &
              \deduce{C \lra C'}{g}
            }
      ...
      \\[0.4cm]
      \begin{array}{c}
        \hspace*{2.5cm}\begin{array}{c}
        \infer[\pi_{\Rarr}]{A \lra B \Rarr C}{
            \infer[\pi^{-1}_{\Rarr}]{A \ot B \lra C}{
                \deduce{A \lra B \Rarr C}{f}
            }
        }
      \end{array}
      \quad \doteq \quad
    ...
    \end{array}$}
  \end{displaymath}
  \caption{Functoriality of \(\Rarr \), naturality and invertibility of $\pi_{\Rarr}$}
  \label{fig:eq:Rarr}
\end{figure}
...
\begin{figure}
  \begin{displaymath}
    \arraycolsep=3pt
    \scalebox{1}{$\begin{array}{l}
      \hspace*{1.3cm}\begin{array}{c}
        \infer[\Larr]{B \Larr A \lra B \Larr A}{
            \infer[\id]{A \lra A}{}
            &
            \infer[\id]{B \lra B}{}
        }
      ...
      \quad \doteq \quad
      \begin{array}{c}
        \infer[\id]{B \Larr A \lra B \Larr A}{}
      \end{array}
      \\[0.4cm]
      \begin{array}{c}
        \infer[\Larr]{D \Larr C \lra F \Larr A}{
            \infer[\comp]{A \lra C}{
                \deduce{A \lra B}{f}
                &
                \deduce{B \lra C}{g}
      ...
      \hspace*{3cm}\doteq
      \begin{array}{c}
        \infer[\comp]{D \Larr C \lra F \Larr A}{
            \infer[\Larr]{D \Larr C \lra E \Larr B}{
                \deduce{B \lra C}{g}
                &
                \deduce{D \lra E}{h}
            }
            &
            \infer[\Larr]{E \Larr B \lra F \Larr A}{
                \deduce{A \lra B}{f}
                &
                \deduce{E \lra F}{k}
            }
      ...
      \\[0.4cm]
      \begin{array}{c}
        \infer[\comp]{A' \lra C \Larr B}{
        \deduce{A' \lra A}{f}
        &
        \infer[\pi_{\Larr}]{A \lra C \Larr B}{
            \deduce{A \ot B \lra C}{g}
        }
      }
      \end{array}
      ...
      \\[0.4cm]
       \begin{array}{c}
        \infer[\pi_{\Larr}]{A \lra (A \ot B') \Larr B}{
            \infer[\ot]{A \ot B \lra A \ot B'}{
                \infer[\id]{A \lra A}{}
                &
                \deduce{B \lra B'}{f}
      ...
      \qquad\qquad \doteq
      \begin{array}{c}
        \infer[\comp]{A \lra (A \ot B') \Larr B}{
            \infer[\pi_{\Larr}]{A \lra (A \ot B') \Larr B'}{
                \infer[\id]{A \ot B' \lra A \ot B'}{}
            }
            &
            \infer[\Larr]{(A \ot B') \Larr B' \lra (A \ot B') \Larr B}{
                \deduce{B \lra B'}{f}
                &
                \infer[\id]{A \ot B' \lra A \ot B'}{}
            }
      ...
      \begin{array}{c}
        \infer[\comp]{A \lra C' \Larr B}{
            ...
            &
            \infer[\Larr]{C \Larr B \lra C' \Larr B}{
                \infer[\id]{B \lra B}{}
                &
                \deduce{C \lra C'}{g}
            }
      ...
      \begin{array}{c}
        \hspace*{2.5cm}\begin{array}{c}
        \infer[\pi_{\Larr}]{A \lra C \Larr B}{
            \infer[\pi^{-1}_{\Larr}]{A \ot B \lra C}{
                \deduce{A \lra C \Larr B}{f}
            }
        }
      \end{array}
      \quad \doteq \quad
    ...
    \end{array}$}
  \end{displaymath}
  \caption{Functoriality of \(\Larr \), naturality and invertibility of $\pi_{\Larr}$}
  \label{fig:eq:Larr}
\end{figure}

\subsection{The Free Magmatic Biclosed Category}
Let $\FMBCC$ (\agdaref{FCatUniversal.agda\#L249}) be the magmatic biclosed category generated by (\ref{eq:axiomatic:calculus}) and the equivalence relation $\doteq$.
Concretely, objects in this category are formulae ($\Fma$) and the set of morphisms between objects $A$ and $B$ is the set of derivations of the sequent $A \lra B$ quotiented by $\doteq$.
The identity morphism is the equivalence class of $\id$, while composition is defined via $\comp$, noticing that this is a well-defined operation since $\doteq$ is a congruence and therefore $\comp$ respects it. The magmatic biclosed structure is straightforward to instantiate.

% We then show that $\FMBCC$ is the free magmatic biclosed category.
The syntactic category $\FMBCC$ is moreover the \emph{free} magmatic biclosed category generated by the set $\At$.  This means that, given another magmatic biclosed category $(\mb{D} , \ot' , \Rarr' , \Larr', \piR', \piL')$ and a valuation function $F_{\mf{At}} : \mf{At} \to |\mb{D}|$ assigning objects of $\mb{D}$ to atomic formulae, there exists a unique strict magmatic biclosed functor $F : \FMBCC \to \mb{D}$ such that $F \circ \mf{at} = F_{\mf{At}}$ (remember that $\mf{at} : \mf{At} \to \Fma$ is the inclusion of atomic formulae into the set of formulae).
Below we provide a sketch of the proof of freeness.

\begin{description}
\item[Existence] (\agdaref{FCatUniversal.agda\#L310})
  The action $F_0$ on objects is defined inductively on formulae:
  \[
  \arraycolsep=10pt
  \begin{array}{cc}
    F_0 X = F_{\mf{At}} X & F_0 (A \ot B) = F_0 A \ot' F_0 B
    \\[5pt]
    F_0 (A \Rarr B) = F_0 A \Rarr' F_0 B & F_0 (B \Larr A) = F_0 B \Larr' F_0 A
  \end{array}
  \]
The action $F_1$ on morphisms is defined by structural recursion on derivations in the homset $A \lra B$. Concretely, each derivation is sent to a morphism in $\mb{D} (F_0 A , F_0 B)$ obtained by replacing $\ot$ by $\ot'$, $\Rarr$ by $\Rarr'$, $\Larr$ by $\Larr'$, $\pi_{\Rarr}$ by $\piR'$ and $\pi_{\Larr}$ by $\piL'$.

Using $F_0$ and $F_1$, we define a functor $F : \FMBCC \to \mb{D}$ which preserves the magmatic biclosed structure strictly.

\item[Uniqueness] (\agdaref{FCatUniversal.agda\#L411})
  Consider another strict magmatic biclosed functor $G : \FMBCC \to \mb{D}$ such that $G \circ \mf{at} = F_{\mf{At}}$, i.e.  $G$ agrees with $F$ on atomic formulae. For each formula $A$, we construct an invertible function $t_A : F~ A\to G~A$ by recursion on $A$ and show that it is a natural transformation by structural recursion on derivations.
  Since $G$ is strict, one can moreover show that $t$ is the identity natural transformation and therefore $F$ and $G$ agree on all objects and morphisms.
% \cheng{maybe some text about why this shows $\FMBCC$ is the free category.}
\end{description}

\subsection{Proof-Relevant Soundness and Completeness}
We have shown that the axiomatic calculus, together with the congruence relation $\doteq$, satisfies a universal property, namely that it underlies the free magmatic biclosed category on the set of atomic formulae $\mf{At}$. 
In other words, the axiomatic calculus is the syntactic incarnation of the categorical semantics.
In this section we show the equivalence between the sequent calculus and the axiomatic calculus for \NL.
The equivalence at the level of \emph{provability}, i.e. that the sequent $A \vdash B$ is provable iff the sequent $A \lra B$ is provable, is well-known \cite{moot:categorial:2012}. Here we extend the equivalence at the level of \emph{proofs}, i.e. there is a bijective correspondence between proofs of $A \vdash B$ modulo $\circeq$ and proofs of $A \lra B$ modulo $\doteq$.
This equivalence can be understood as a proof-relevant soundness and completeness theorem for the sequent calculus wrt.\ the categorical models of Section \ref{sec:cat:sem}.

Explicitly, the equivalence between the sequent calculus and the axiomatic calculus is proved by constructing two functions $\sound$ (\agdaref{Sound.agda\#L63}) and $\complt$ (\agdaref{Complt.agda\#L15}) typed
\[
\sound : T \vd C \to T^* \lra C,
\qquad
\complt : A \lra C \to A \vd C
\]
where, given a tree $T$, the formula $T^*$ is obtained by replacing each comma in $T$ by a tensor $\ot$.
These functions satisfy the following properties:
  \begin{enumerate}[label=($\roman*$)]
    \item for $f , g : T \vd C$, if $f \circeq g$, then $\sound ~ f \doteq \sound ~ g$ (\agdaref{Sound.agda\#L75});
    \item for $f , g : A \lra C$, if $f \doteq g$, then $\complt ~ f \circeq \complt ~ g$ (\agdaref{Complt.agda\#L43});
    \item for $f : T \vd C$, $\complt ~(\sound ~ f) \circeq \tl^* ~ f$ (\agdaref{CompltSound.agda\#L26});
    \item for $f : A \lra C$, $\sound ~(\complt ~ f) \doteq f$ (\agdaref{SoundComplt.agda\#L228}).
  \end{enumerate}
  In property $(iii)$ above, $\tl^*$ is a derivable rule obtained by iterated applications of $\tl$:
\[
\infer[\tl^*]{T^* \vd C}{
  T \vd C
}
...
The functions $\sound$ and $\complt$ are both constructed by structural recursion on derivations.
We show the definition of $\sound$ when the input derivation ends with an application of a $\Rl$-rule:
  \[
  \scalebox{0.95}{$
  \begin{array}{l}
    \sound ~\left(
    ...
    \quad
    \proofbox{
      \infer={\mc{C}[U , A \Rarr B]^* \lra C}{
      \infer[\comp]{\mc{C}[U^* \ot (A \Rarr B)]^* \lra C}{
        \infer[\mf{cxt}]{
          \mc{C}[U^* \ot (A \Rarr B)]^* \lra \mc{C} [B]^*
        }{
          \infer[\comp]{U^* \ot (A \Rarr B) \lra B}{
            \infer[\ot]{U^* \ot (A \Rarr B) \lra A \ot (A \Rarr B)}{
              \deduce{U^* \lra A}{\sound ~ f}
              &
              \infer[\id]{A \Rarr B \lra A \Rarr B}{}
            }
            &
            \infer[\piRm]{A \ot (A \Rarr B) \lra B}{
              \infer[\id]{A \Rarr B \lra A \Rarr B}{}
            }
          }
        }
  ...
  $}
  \]
  %  We show one interesting case of $\sound$ where the admissible rule (\ref{lem:f2Tf}) is essential.
  The derivation above uses the rule $\mf{cxt}$ (\agdaref{Sound.agda\#L13}), which is admissible in the axiomatic calculus and states the monotonicity of the operation $A \mapsto \mc{C}[A]^*$ for each context $\mc{C}$:
  %It states that for any context $\mc{C}$ and derivation $f : A \lra B$  where ${-}^* : \Tree \to \Fma$ is a function that replaces all commas by $\ot$ in a tree:
% (\agdaref{Sound.agda\#L13})
  \begin{equation}\label{lem:f2Tf}
    \infer[\mf{cxt}]{\mc{C}[A]^* \lra \mc{C}[B]^*}{
      A \lra B
      }
  \end{equation}
  ...
  Properties $(i)$ and $(ii)$ are formalized in Agda as the following functions:
  \[
    \begin{array}{l}
      {\circeq}\sound{\doteq} : \{f ~ g : T \vd C\} ~ (eq : f \circeq g) \to \sound ~ f \doteq \sound ~ g
     \\[2pt]
      {\doteq}\complt{\circeq} : \{f ~ g : A \lra B\} ~ (eq : f \doteq g) \to \complt ~ f \circeq \complt ~ g
     \end{array}
    \]
  They are both constructed by patter-matching on the argument $eq$, i.e. the proof of equivalence of derivations. 


  The construction of the function ${\circeq}\sound{\doteq}$ relies on several properties of the admissible rule $\mf{cxt}$ in (\ref{lem:f2Tf}):
  %, which are collected in Figure \ref{fig:cxt}.
  %From top to bottom,  these properties state that:
  \begin{itemize}
  \item $\mf{cxt}$ preserves composition $\comp$ (\agdaref{Sound.agda\#L29}):
\[
\proofbox{
  \infer[\mf{cxt}]{\mc{C}[A]^* \lra \mc{C}[C]^*}{
    \infer[\comp]{A \lra C}{
      \deduce{A \lra B}{f}
      &
      \deduce{B \lra C}{g}
...
\doteq
\proofbox{
  \infer[\comp]{\mc{C}[A]^* \lra \mc{C}[C]^*}{
    \infer[\mf{cxt}]{\mc{C}[A]^* \lra \mc{C}[B]^*}{
      \deduce{A \lra B}{f}
    }
    &
    \infer[\mf{cxt}]{\mc{C}[B]^* \lra \mc{C}[C]^*}{
      \deduce{B \lra C}{g}
    }
  }
}
\]    
\item  $\mf{cxt}$ applied to a composite context results in two applications of $\mf{cxt}$ (\agdaref{Sound.agda\#L37}):
  \[
  \proofbox{
  \infer={\mc{C}[\mc{C'}[A]]^* \lra \mc{C}[\mc{C'}[B]]^*}{
    \infer[\mf{cxt}]{(\mc{C}[\mc{C'}])[A]^* \lra (\mc{C}[\mc{C'}])[B]^*}{
      \deduce{A \lra B}{f}
    }
  }
}
...
\]
In the first derivation, the context $\mc{C}[\mc{C'}]$ is obtained by plugging the context $\mc{C'}$ in a hole of $\mc{C}$.
\item Two applications of $\mf{cxt}$ on disjoint parts of the context can be swapped (\agdaref{Sound.agda\#L44}):
  \[
  \begin{array}{c}
\proofbox{
  \infer={(\mc{C}[A])[A']^* \lra (\mc{C}[B])[B']^*}{
    \infer[\mf{comp}]{(\mc{C}[A'])[A]^* \lra (\mc{C}[B'])[B]^*}{
      \infer[\mf{cxt}]{(\mc{C}[A'])[A]^* \lra (\mc{C}[A'])[B]^*}{
        \deduce{A \lra B}{f}
      }
      &
      \infer[\mf{cxt}]{(\mc{C}[A'])[B]^* \lra (\mc{C}[A'])[B']^*}{
        \deduce{A' \lra B'}{g}
      }
    }
  }
...
\proofbox{
  \infer[\mf{comp}]{(\mc{C}[A])[A']^* \lra (\mc{C}[B])[B']^*}{
    ...
    &
    \infer={(\mc{C}[A])[B']^* \lra (\mc{C}[B])[B']^*}{
      \infer[\mf{cxt}]{(\mc{C}[B'])[A]^* \lra (\mc{C}[B'])[B]^*}{
        \deduce{A \lra B}{f}
      }
    }
  }
...
\end{array}
\]    
\item $\mf{cxt}$ respects the congruence relation $\doteq$ (\agdaref{Sound.agda\#L54}).
  \end{itemize}
  The construction of the function ${\doteq}\complt{\circeq}$ heavily relies on  $\cut{\circeq}_1$ and $\cut{\circeq}_2$, i.e.\ the fact that the admissible cut rule respects $\circeq$ in each of its arguments.
...
%\end{array}
%\]
%\caption{Properties of the admissible rule $\mf{cxt}$}
%\label{fig:cxt}
%%\end{landscape}
%\end{figure}
  
  
  Properties $(iii)$ and $(iv)$ are formalized in Agda as the following functions:
  \[
  \begin{array}{l}
      \mf{compltsound} : (f : T \vd C) \to \complt ~(\sound ~ f) \circeq \tl^* f
    \\[2pt]
      \mf{soundcomplt} : (f : A \lra B) \to \sound ~(\complt ~ f) \doteq f
  \end{array}
  \]
  These two properties are both defined by pattern-matching on $f$. 
  In the definition of $\mf{soundcomplt}$ we employ a lemma $\mf{soundcut}$ (\agdaref{SoundComplt.agda\#L44}), which states that the function $\sound$ preserves morphism composition: it sends a composite (i.e. obtained via an application of the admissible $\cut$ rule) derivation in the sequent calculus to a composite (i.e. obtained via an application of the primitive $\comp$ rule) derivation in the axiomatic calculus.
  % Notice that $\tl$ is an invertible rule, i.e.\ the following rule is derivable:
  % \[
  % \infer[\tl^{-1}]{\mc{C}[A , B] \vd C}{\mc{C}[A \ot B] \vd C}
  % \]
  % Moreover, $\tl$ and $\tl^{-1}$ are each other's inverses wrt. $\doteq$.
  % Therefore, it is provable that 
  % This is essential for soundcomplt
  % soundcut : ∀ {T U W C D} (p : Path T)
  % → (f : U ⊢ D) (g : W ⊢ C)
  % → (eq : W ≡ sub p (η D))
  % → sound (cut p f g eq) ≐ (sound g ∘ (subst-ant (cong [_] eq) id ∘ f2T[f] {U = U} {η D} p (sound f)))

\section{Proof-Relevant Maehara Interpolation}\label{sec:proof:rel:mip}
%In this section, we introduce three aspects of interpolation for nonassociative Lambek calculus: Maehara interpolation, proof-relevant interpolation, and showing that Maehara interpolation is well-defined wrt.\ equivalence of derivations.
...
Maehara interpolation for the sequent calculus of \NL\ has the following form: given a derivation $f : \mc{C}[U]\vd C$, there exist a formula $D$ and two derivations $g : \mc{C}[D] \vd C$ and $h : U \vd D$ such that $\vars{D} \subseteq \vars{U} \cap \vars{\mc{C}, C}$.
...
Consider the case where $f : \mc{C}[U]\vd C$ ends with an application of the rule $\Rl$, i.e.
\[
f =
\proofbox{
  \infer[\Rl]{\mc{C'}[W , A \Rarr B] \vd C}{
    \deduce{W \vd A}{f'}
    &
    \deduce{\mc{C'}[B] \vd C}{f''}
  }
}
\] 
which forces an equality of trees $\mc{C'}[W , A \Rarr B] = \mc{C}[U]$.
In the Agda formalization, this equality appears as the type $\sub{p'}{(W \cdast \eta ~(A \Rarr B))} \equiv \sub{p}{U}$ of the argument $eq$, for some paths $p$ and $p'$ specifying the hole in contexts $\mc{C}$ and $\mc{C'}$, respectively.
We can apply our lemma $\subeq$ to the equality proof $eq$ to give rise to the following seven possible cases (\agdaref{Interpolation.agda\#L29}):
\begin{itemize}
  \item[--] $\oneeqtwo$, where $U$ is syntactically equal to $(W, A \Rarr B)$;
  \item[--] $\twogtLone$ ($\twogtRone$), where $U$ contains $(W, A \Rarr B)$ in its left (right) subtree;
  \item[--] $\onegtLtwo$, where $(W, A \Rarr B)$ contains $U$ in its left subtree, i.e.\ $U$ is in $W$;
  \item[--] $\onegtRtwo$, where $(W, A \Rarr B)$ contains $U$ in its right subtree, i.e.\ $U = A \Rarr B$;
  \item[--] $\oneLtwoR$ and $\oneRtwoL$, where $U$ are $(W, A \Rarr B)$ are disjoint subtrees.
\end{itemize}
The interesting case is $\onegtRtwo$, where we apply inductive hypothesis on $f'$ and $f''$ with the interpolant tree $W$ and $B$, respectively.
The resulting interpolant triples are $(D , g' : D \vd A , h' : W \vd D)$ and $(E , g'' : \mc{C'}[E] \vd C , h'' : B \vd E)$. The desired interpolant formula is $D \Rarr E$ and the desired derivations are constructed as follows:
\[
\begin{array}{c}
  g =
  \proofbox{
    \infer[\Rl]{\mc{C'}[W , D \Rarr E] \vd C}{
      \deduce{W \vd D}{h'}
      &
      \deduce{\mc{C'}[E] \vd C}{g''}
    }
  }
  \qquad
  h =
  \proofbox{
    \infer[\Rr]{A \Rarr B \vd D \Rarr E}{
      \infer[\Rl]{D , A \Rarr B \vd E}{
        \deduce{D \vd A}{g'}
        &
        \deduce{B \vd E}{h''}
      }
...
\end{array}
\]
The corresponding snippet in Agda is
\[
\begin{array}{rl}
  \;\; \quad \dots &\vert ~ \onegtRtwo ~ (\containsright ~\bullet ~ \refl ~ \refl ~ \refl) = \\[2pt]
  \;\; \quad &\mf{let} ~\mf{intrp} ~ D ~ g' ~ h' = \mf{mip} ~ \bullet ~ W ~ f' ~ \refl \\[2pt]
  \;\; \quad & \quad \ \mf{intrp} ~ E ~ g'' ~ h'' = \mf{mip} ~ p' ~ (\eta ~ B) ~ f'' ~ \refl \\[2pt]
  \;\; \quad & \mf{in} ~ \mf{intrp} ~(D \Rarr E) ~ (\Rl ~ p' ~ h' ~g'') ~ (\Rr ~(\Rl ~ \bullet ~ g' ~ h''))
\end{array}
\]
where $\mf{intrp}$ is the constructor of the record type $\MIP$ and $\containsright$ is the constructor of the record type $\ContainsRight$.
...
\begin{array}{rl}
  ...
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad varg &: \forall \{X\} \to X \in D \to X \inT T \uplus X \in C \\[2pt]
...
%  \multicolumn{2}{l}{\quad \mf{constructor} ~ \mf{intrp}{\circeq}} \\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad eqD &: D \equiv D' \\[2pt]
  \;\; \quad eqg &: \subst ~(\lambda x \to \sub{p}{(\eta ~ x)} \vd C)~eqD~g \circeq g' \\[2pt]
  \;\; \quad eqh &: \subst ~(\lambda x \to U \vd x) ~ eqD ~ h \circeq h'
\end{array}
\]
%Elements of type $\MIP ~ p ~ U ~C$ are triples consisting of a formula $D$ and two derivations $g$ and $h$.
...
\end{array}
\]
An interesting case in the proof arises when $eq_1$ is the permutative conversion of disjoint applications of $\Rl$:
\[
\scalebox{0.98}{$
\proofbox{
      \infer[\Rl]{\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']] \vd C}{
        \deduce{V \vd A}{f}
        & \!\!\!
        \infer[\Rl]{\mc{C'}[\mc{C}_1[B] , \mc{C}_2[W , A' \Rarr B']] \vd C}{
          \deduce{W \vd A'}{f'}
          & \!\!
          \deduce{\mc{C'}[\mc{C}_1[B] , \mc{C}_2[B']] \vd C}{g}
        }
    ...
    \proofbox{
      \infer[\Rl]{\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']] \vd C}{
        \deduce{W \vd A'}{f'}
        & \!\!\!
        \infer[\Rl]{\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[B']] \vd C}{
          \deduce{V \vd A}{f}
          & \!\!
          \deduce{\mc{C'}[\mc{C}_1[B] , \mc{C}_2[B']] \vd C}{g}
        }
    ...
    $}
\]
This forces an equality of trees $\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']] = \mc{C}[U]$, that in Agda is an equality type of the form: 
\[
eq : \sub{p'}{(\sub{p_1}{(V \cdast A \Rarr B)} , \sub{p_2}{(W \cdast A' \Rarr B')})} \equiv \sub{p}{U}
\]
In this case, we apply the lemma $\subeq$ to the equality proof $eq$ and we are given seven possible cases.
However, in some cases, e.g. $\twogtLone$ where $\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']]$ contains $U$ in its left subtree, we have to apply the lemma $\subeq$ another time to the equality proof $eq' : \sub{p_1}{(V \cdast A \Rarr B)} \equiv \sub{q}{U}$ (which appears in scope as a projection of record type $\ContainsLeft$), which gives rise to seven more cases, and similarly for $\twogtRone$.
This showcases yet again the value of proof assistants to keep track of all cases and proof obligations.
...
\circeq 
\subst~ (\lambda x \to x \vdash C)~eq~f
\end{array}
\]
...
We have presented an Agda formalization of \NL, showcasing the proofs of the following properties: cut admissibility, proof-relevant Maehara interpolation and soundness and completeness wrt.\ magmatic biclosed categories. The key technical contribution is the formal characterization of equality of substituted trees, which serves as a fundamental lemma for the formalization of the aforementioned properties.
For the future, we expect to proving other properties, such as uniform interpolation \cite{alizadeh:2014} for an extension of \NL\ with additives, with the fundamental lemma.
While cut admissibility and the standard Maehara interpolation property for the sequent calculus of \NL\ have been established in previous research, our formalization also offers some new theoretical results: a proof-relevant version of Maehara interpolation and the well-definedness of $\cut$ and $\mf{mip}$ wrt.\ permutative conversions.
%interpolation First, our Agda formalization of the case distinction type for trees provides a rigorous foundation that can be reused in further research such as uniform interpolation \cite{alizadeh:2014}. Second, our proof-relevant interpolation results, demonstrating that the Maehara interpolation procedure is well-defined wrt.\ equivalence of derivations, is new theoretical result of \NL.

For many substructural logics, including \NL, the variable condition in the Maehara interpolation property can be strengthened to include the multiplicity \cite{moot:categorial:2012} and the polarity \cite{roorda1991} of occurrences of atomic formulae. We plan to formalize these upgrades of the variable condition $\mf{VarCond}$ in future work.

The well-definedness of $\mf{mip}$ wrt.\ $\circeq$ is attainable in \NL, but it cannot be proved for more structured logics, e.g.\ the associative Lambek calculus, or even a semi-associative fragment \cite{VW2025}, without relaxing the equivalence of interpolation triples. In particular, one cannot expect the interpolant formulae to be syntactically equal, as in our type $\MIPeq ~ m_1 ~ m_2$.
We have found concrete examples in the associative Lambek calculus where the interpolant formulae are neither equal nor logically equivalent.
These examples suggest that a plausible relaxation would be a ``zig-zag'' equivalence, which means that two interpolant triples are equivalent if there exists a zig-zag chain between them.
...
We plan to work on the code in order to reduce the compilation time in the future.
