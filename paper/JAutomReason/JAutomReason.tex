...
\begin{array}{ll}
  ...
  \;\; \quad \Rarro &: \{A ~ B ~ C ~ D : \Obj\} \to \Homs{B}{A} \to \Homs{C}{D} \\
...
There are record fields (not shown below) stating that $\mf{Eq}$ is an equivalence relation and that is moreover a congruence relation wrt. operations $\mf{Comp}$, $\oto$, $\Rarro$, $\Larro$, $\pi_{\mf{R}}$, $\pi^{-1}_{\mf{R}}$, $\pi_{\mf{L}}$ and $\pi^{-1}_{\mf{L}}$.
...
\begin{array}{ll}
  ...
  \;\; \quad \piRm\piR &: \{A ~ B ~ C : \Obj\} ~ \{f : \Homs{(\otz ~ A ~ B)}{C}\} \to \Eq{(\piRm (\piR ~ f))}{f}\\[2pt]
  \;\; \quad \piR\piRm &: \{A ~ B ~ C : \Obj \} ~ \{f : \Homs{B}{(\Rarrz A ~ B)}\} \to \Eq{(\piR (\piRm ~ f))}{f}\\[2pt]
  \;\; \quad \piR A &: \{A ~ B ~ A' : \Obj\} ~ \{f : \Homs{A'}{A}\}  \\
  \;\; \quad &\hspace*{0.5cm}\to \Eq{(\piR (\oto ~ f ~ \mf{Id}))}{(\mf{Comp} ~ (\Rarro f ~ \mf{Id}) ~ (\piR ~ \mf{Id}))}\\[2pt]
  \;\; \quad \piR B &: \{A ~ B ~ C ~ B' : \Obj\} ~ \{f : \Homs{(\otz ~ A ~ B)}{C}\} ~ \{g : \Homs{B'}{B}\} \\
  \;\; \quad &\hspace*{0.5cm}\to \Eq{(\mf{Comp} ~ (\piR ~ f) ~ g)}{\piR ~ (\mf{Comp} ~ f ~ (\oto ~ \mf{Id} ~ g))} \\[2pt]
  \;\; \quad \piR C &: \{A ~ B ~ C ~ C' : \Obj\} ~ \{f : \Homs{C}{C'}\} ~ \{g : \Homs{(\otz ~ A ~ B)}{C}\} \\
  \;\; \quad &\hspace*{0.5cm} \to \Eq{(\piR ~ (\mf{Comp} ~ f ~ g))}{(\mf{Comp} ~ (\Rarro \mf{Id} ~ f) (\piR ~ g))}
\end{array}
\]
...
\begin{array}{ll}
  ...
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad F_0 &: \Obj ~ \mb{C} \to \Obj ~ \mb{D} \\[2pt]
  \;\; \quad F_1 &: \forall \{B ~ C\} \to \HomC{C}{B}{C} \to \HomC{D}{F_0 B}{F_0 C}\\[2pt]
  \;\; \quad m &: \forall\{B ~ C\} \to \EqC{D}{(\otz ~ \mb{D} ~ (F_0 B) ~ (F_0 C))}{(F_0 (\otz ~ \mb{C} ~ B ~ C))} \\[2pt]
  \;\; \quad m^{-1} &: \forall\{B ~ C\} \to \EqC{D}{(F_0 (\otz ~ \mb{C} ~ B ~ C))}{(\otz ~ \mb{D} ~ (F_0 B) ~ (F_0 C))} \\[2pt]
  \;\; \quad nm &: \forall \{B ~ C ~ D ~ E\} ~ \{f : \HomC{C}{B}{D}\} ~ \{\HomC{C}{C}{E}\} \\
  \;\; \quad &\quad \to \EqC{D}{(\CompC{D}{m}{(\oto ~ \mb{D} ~ (F_1 f) ~ (F_1 g))})}{(\CompC{D}{(F_1 (\oto ~ \mb{C} ~ f ~ g))}{m})}\\[2pt]
  \;\; \quad miso_{1} &: \forall \{B ~ C\} \to \EqC{D}{(\CompC{D}{m}{m^{-1}})}{(\IdC{D})} \\[2pt]
  \;\; \quad miso_{1} &: \forall \{B ~ C\} \to \EqC{D}{(\CompC{D}{m^{-1}}{m})}{(\IdC{D})} \\[2pt]
  \;\; \quad pR &: \forall \{A ~ B ~ C\} ~ \{f : \HomC{C}{(\otz ~ \mb{C} ~ A ~ B)}{C}\} \\
  \;\; \quad &\quad \to \EqC{D}{(\CompC{D}{r}{(F_1 (\piR ~ \mb{C} ~ f))})}{(\piR ~ \mb{D} ~ (\CompC{D}{(F_1 f)}{m}))}
\end{array}
$}
\]
