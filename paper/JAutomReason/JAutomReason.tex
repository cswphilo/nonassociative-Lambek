...
The original proposal by Lambek \cite{lambek1988} includes an axiom of identity, a transitivity rule and four rules evidencing the adjunctions between tensor and the two implications (cf. Definition \ref{def:mbcc}).
...
To interpret derivations as morphisms, we define an equivalence relation $\doteq$ on derivations (Figures \ref{fig:cat:eq},\ref{fig:eq:Rarr} and \ref{fig:eq:Larr}) that respects the definition of a category and moreover the magmatic biclosed structure in Definition \ref{def:mbcc}.
...
\subsection{Categorical Semantics}\label{sec:cat:sem}
In this section we present the categorical models of \NL, magmatic biclosed categories and then prove that the axiomatic calculus (\ref{eq:axiomatic:calculus}) with the equivalence relation $\doteq$ generates the free magmatic biclosed category.
\begin{definition}\label{def:mbcc}
  Magmatic biclosed category \cite{Wijnholds2017} $\mb{C}$ is a category equipped with three functors $\ot : \mb{C} \times \mb{C} \to \mb{C}$, $\Rarr: \mb{C}^{op} \times \mb{C} \to \mb{C}$ and $\Larr: \mb{C} \times \mb{C}^{op} \to \mb{C}$ form two adjunctions $B \ot {-} \dashv B \Rarr {-}$ and ${-} \ot B \dashv {-} \Larr B$ both natural in $B$.
\end{definition}
A magma is a monoid without unit and associativity properties, so a magmatic category can be thought of as a monoidal category without the unit and the coherence laws of unitality and associativity. The biclosed structure captures the two implications in \NL.

We implement magmatic biclosed category in Agda as a record type that contains three groups of data.
The first group defines objects and actions on objects (\agdaref{FCatUniversal.agda\#L11}):
 \[
\begin{array}{ll}
  \multicolumn{2}{l}{\record \:\: \MBCC : \Set_{\mf{1}} \:\: \where} \\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad \Obj &: \Set \\[2pt]
  \;\; \quad \otz &: \Obj \to \Obj \to \Obj \\[2pt]
  \;\; \quad \Rarrz &: \Obj \to \Obj \to \Obj \\[2pt]
  \;\; \quad \Larrz &: \Obj \to \Obj \to \Obj
\end{array}
\]
The second group defines the morphisms and actions on morphism (\agdaref{FCatUniversal.agda\#L20}).
Morphisms are defined as pairs over object. Here we additionally show five constructors which correspond to the ones in (\ref{eq:constructor:axiomatic:calc}):
\[
\begin{array}{ll}
  \;\; \quad \Hom &: \Obj \to \Obj \to \Set \\[2pt]
  \;\; \quad \mf{Id} &: \{B : \Obj\} \to \Homs{B}{B} \\[2pt]
  \;\; \quad \mf{Comp} &: \{D ~ B ~ C : \Obj\} \to \Homs{B}{C} \to \Homs{D}{B} \to \Homs{D}{C} \\[2pt]
  \;\; \quad \Rarro &: \{A ~ B ~ C ~ D : \Obj\} \to \Homs{B}{A} \\
  \;\; \quad &\hspace*{0.5cm}\to \Homs{C}{D} \to \Homs{(\Rarrz A ~ C)}{(\Rarrz B ~ D)} \\[2pt]
  \;\; \quad \pi_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{(\otz ~ A ~ B)}{C} \to \Homs{B}{(\Rarrz A ~ C)} \\[2pt]
  \;\; \quad \pi^{-1}_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{B}{(\Rarrz A ~ C)} \to \Homs{(\otz ~ A ~ B)}{C} 
\end{array}
\]
The parentheses are not part of the syntax. We include them here for better readability.

The third group defines the equalities between morphisms, which first states that the equality on morphisms is an equivalence relation, the congruence laws for $\mf{Comp}$ and actions on morphisms, unitality and associativity of $\mf{Comp}$, functoriality of $\otz$, $\Rarrz$ and $\Larrz$, $\pi_i$ and $\pi^{-1}_i$ being each other's inverses, for $i \in \{\mf{R} , \mf{L} \}$ and naturality of adjunctions.
Here we show the constructors of $\pi_{\mf{R}}$ and $\pi^{-1}_{\mf{R}}$ being each other's inverses (\agdaref{FCatUniversal.agda\#L70}) and naturalities of $\pi_{\mf{R}}$ (\agdaref{FCatUniversal.agda\#L80}).
\[
\begin{array}{ll}
...
\begin{definition}
Let $(\mb{C} , \ot , \Rarr , \Larr)$ and $(\mb{D} , \ot' , \Rarr' , \Larr')$ be magmatic biclosed categories. A strict magmatic biclosed functor $F : \mb{C} \to \mb{D}$ is a functor satisfying $F (A \ot B) = FA \ot' FB$, $F (A \Rarr B) = FA \Rarr' FB$ and $F(B \Larr A) = FB \Larr' FA$.
\end{definition}
...
\begin{definition}
  We define a function ${-}^* : \Tree \to \Fma$ that replaces all commas by $\ot$ in a tree (\agdaref{Sound.agda\#L13}).
\end{definition}

\begin{lemma}\label{lem:f2Tf}
  The following rule is admissible for any $\mc{C}[\bullet]$ and $f : A \lra B$ (\agdaref{Sound.agda\#L21}):
  \[
...
\begin{theorem}\label{thm:equiv:calculi}
...
\begin{proof}
  ...
  We show one interesting case of $\sound$ where Lemma \ref{lem:f2Tf} is essential.
  \[
  \begin{array}{l}
    \proofbox{
      \infer[\Rl]{\mc{C}[U , A \Rarr B] \vd C}{
  ...
  The first two properties are implemented as the following functions in Agda, respectively:\[
    ...
    \end{array}
    \]
  They are both defined by patter-matching on $eq$, the proof of equivalence of derivations. 
  The definition of ${\circeq}\sound{\doteq}$ relies on several properties of the admissible rule defined in Lemma \ref{lem:f2Tf}, including well-behaved wrt. $\comp$ (\agdaref{Sound.agda\#L29}), path concatenation (\agdaref{Sound.agda\#L37}), disjoint instantiation (\agdaref{Sound.agda\#L44}), and congruence wrt. $\doteq$ (\agdaref{Sound.agda\#L54}).
