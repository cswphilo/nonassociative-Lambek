...
\abstract{This paper presents a formalization of the nonassociative Lambek calculus in the Agda proof assistant. %, focusing on two major proof-theoretic properties: cut admissibility and Maehara interpolation. 
  The sequent calculus for this logic has sequents with binary trees as antecedents, in which formulae are stored as leaves. The structure of the antecedents creates subtleties when proving logical properties since in many cases one needs to analyze equalities of different presentations of the same tree. 
We formally characterize all the cases of how two presentations of a tree can be equal in Agda.
The resulting technical lemma helps us to exhaust all possible cases when proving several meta properties of the calculus, including cut admissibility and the Maehara interpolation property which implies Craig interpolation.
...
\begin{equation}\label{eq:adj}
  A \ot {-} \dashv A \Rarr {-}, \qquad {-} \ot A \dashv {-} \Larr A
\end{equation}
both natural in $A$.
...
There is an identity morpshism $\mf{Id}$ for each object, as well as a composition operation $\mf{Comp}$.
...
\end{array}
\]
The implemenation of categories using \emph{setoids} for homsets, i.e. using types $\Homs{A}{B}$ each equipped with a congruence relation $\mf{Eq}$, is standard in type theory, see e.g. \cite{HuC21}.
...
%In the context of monoidal (closed) functors, the left to right direction of $m$ ($r$ or $l$) together with the commutative diagram of associativity constitutes the $\ot$ ($\Rarr$ or $\Larr$) part of a lax monoidal (clsoed) functor. The other direction is called a oplax monoidal (closed) functor.
...
There exist various equivalent presentations of the axiomatic calculus for \NL\ in the literature, their relationship is thoroughly analized in \cite[Ch.~4]{moot:categorial:2012}. 
...
Their version contains both the monotonicity and adjunction rules, but no transivity rule, which they show to be admissible.
...
Figure \ref{fig:eq:Larr} contains similar equations to those in Figure \ref{fig:eq:Rarr} but for $\Larr$ insetad of $\Rarr$.
...
In particular, we need to remember to add rules for reflexivity, symmetry and transitivity of $\doteq$, as well as congurence rules for all constructors.
\begin{figure}[t]
  % \vspace{0.5\textwidth}
%\begin{landscape}
\[
...
\begin{figure}
  \begin{displaymath}
    ...
    \scalebox{1}{$\begin{array}{c}
      ...
      \\[0.4cm]
      \begin{array}{c}
        \infer[\comp]{A \lra B  \Rarr C}{
        \deduce{A \lra A'}{f}
        &
        \infer[\pi_{\Rarr}]{A' \lra B \Rarr C}{
            \deduce{A' \ot B \Rarr C}{g}
        }
      }
      \end{array}
      \hspace*{4cm}
      ...
      \begin{array}{c}
        \infer[\comp]{A \lra B \Rarr C'}{
            ...
            &
            \infer[\Rarr]{B \Rarr C \lra B \Rarr C'}{
                \infer[\id]{B \lra B}{}
                &
                \deduce{C \lra C'}{g}
            }
      ...
      \\[0.4cm]
        \begin{array}{c}
        \infer[\pi_{\Rarr}]{A \lra B \Rarr (A \ot B')}{
            \infer[\ot]{A \ot B \lra A \ot B'}{
                \infer[\id]{A \lra A}{}
                &
                \deduce{B \lra B'}{f}
      ...
      \qquad\qquad \doteq
      \begin{array}{c}
        \infer[\comp]{A \lra B \Rarr (A \ot B')}{
            \infer[\pi_{\Rarr}]{A \lra B' \Rarr (A \ot B')}{
                \infer[\id]{A \ot B' \lra A \ot B'}{}
            }
            &
            \infer[\Rarr]{B' \Rarr (A \ot B') \lra B \Rarr (A \ot B')}{
                \deduce{B \lra B'}{f}
                &
                \infer[\id]{A \ot B' \lra A \ot B'}{}
            }
...
\begin{figure}
  \begin{displaymath}
    ...
    \scalebox{1}{$\begin{array}{c}
      ...
      \\[0.4cm]
      \begin{array}{c}
        \infer[\comp]{A \lra B  \Larr C}{
        \deduce{A \lra A'}{f}
        &
        \infer[\pi_{\Larr}]{A' \lra B \Larr C}{
            \deduce{A' \ot C \Larr B}{g}
        }
      }
      \end{array}
      \hspace*{4cm}
...
The equivalence at the level of \emph{provability}, i.e. that the sequent $A \vdash B$ is provable iff the sequent $A \lra B$ is provable, is well-known \cite{moot:categorial:2012}. Here we extend the equivalence at the level of \emph{proofs}, i.e. there is a bijetive correspondece between proofs of $A \vdash B$ modulo $\circeq$ and proofs of $A \lra B$ modulo $\doteq$.
