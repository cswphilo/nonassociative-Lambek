...
\]
which tells us an equation $\mc{C'}[\mc{C}_1[U , A \Rarr B] , \mc{C}_2[V , A' \Rarr B']] = \mc{C}[U]$ that turns out to be the type $\sub{p'}{(\mc{C}_1[U , A \Rarr B] , \mc{C}_2[V , A' \Rarr B'])} \equiv \sub{p}{U}$ in Agda.
In this case, we apply the lemma $\subeq$ to the equality proof $eq$ to distinguish seven cases of the equation.
However, in some cases, e.g. $\mc{C'}[\mc{C}_1[U , A \Rarr B] , \mc{C}_2[V , A' \Rarr B']]$ contains $U$ in its left or right subtree, we have to apply the lemma $\subeq$ the new equality proof $eq' : \sub{p_1}{\mc{C}_1[U , A \Rarr B]}$ (or $\sub{p_2}{\mc{C}_2[V , A' \Rarr B']}$) to further distinguish another seven cases.
