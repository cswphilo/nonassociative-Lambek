...
We additionally prove the proof-relevant versions of ($i$) soundness and completeness with respect to the Hilbert-style (axiomatic) calculus and ($ii$) Maehara interpolation, i.e.\ the interpolation procedure as a right inverse of the admissible cut rule. }
...
Next, we present the axiomatic (Hilbert-style) calculus of \NL. The first axiomatic calculus of \NL\ was first proposed by Lambek \cite{lambek1988} for linguistic analysis.
...
The presentation we chose is a direct characterization of the categorical models of \NL, the magmatic biclosed categories \cite{Wijnholds2017}, which should be thought of as a monoid without unit and properties of associativity.
We first formalize the axiomatic calculus and the definition of magmatic biclosed categories.
Then we show that the axiomatic calculus with its own equivalence relation on of derivations $\doteq$ generates the free magmatic biclosed category.
Moreover, we show that the axiomatic and sequent calculi are equivalent wrt. proofs, i.e.\ there exists a bijection function between the derivations of the two calculi that preserves the equivalence of derivations.
...
This paper is an extended version of \cite{VW2025Tableaux}. The whole Section \ref{sec:axiomatic} on the axiomatic (Hilbert-style) calculus of \NL, is new. It includes categorical models of \NL\ and equivalence between axiomatic and sequent calculi.
...
\section{Axiomatic Calculus of \NL}\label{sec:axiomatic}
The axiomatic (Hilbert-style) calculus of \NL\ proposed by Lambek \cite{lambek1988} has one axiom of identity, four rules of adjunctions (cf. Definition \ref{def:mbcc}) and one transitivity rule.
By contrast, Do{\v s}en's calculus \cite{Dosen1988}, while also including the identity axiom and the transitivity rule, replaces the adjunction rules by four axioms of units and counits of the adjunctions and three functorial rules of the connectives $\ot$, $\Rarr$ and $\Larr$.
Our formulation is closer to the one proposed by Moortgat and Oehrle \cite{moot:categorial:2012}, which contains both the functorial and adjunction rules, but we add the transitivity rule (morphism composition) to make it a direct characterization of the categorical models.
...
Formulae in the axiomatic calculus are inductively generated by the same grammar in the beginning of Section \ref{sec:fma:tree} while the derivations are inductively generated by the following rules:
...
% \]
\end{equation}
The double lines in $\pi_{\Rarr}$ and $\pi_{\Larr}$ means that the rules are invertible. The bottom-up inference rules are denoted as $\pi^{-1}_{\Rarr}$ and $\pi^{-1}_{\Larr}$, respectively.
...
\subsection{Proof-Relevant Equivalence}
...
In this section we show the equivalence between the sequent (\ref{eq:seqcalc}) and axiomatic (\ref{eq:axiomatic:calculus}) calculi of \NL\ wrt.\ proofs, which, in other words, can be understood as soundness and completeness wrt.\ categorical models.
