...
Do{\v s}en \cite{Dosen1988} and Moortgat and Oehrle \cite{moot:categorial:2012}\footnote{\cheng{The book by Moot and Retore mentioned this paper but I can't find it. I am not sure if we should cite the original paper or their book.}} also proposed different axiomatic formulations with applications in soundness and completeness wrt.\ ternary relational models and complexity of \NL, respectively.
...
We have shown that the axiomatic calculus (\ref{eq:axiomatic:calculus}) with $\doteq$ generates the free magmatic biclosed category. 
In other words, we can think of it as a syntactic characterization of the categorical models. 
In this section we show the equivalence between the sequent (\ref{eq:seqcalc}) and axiomatic (\ref{eq:axiomatic:calculus}) calculi for \NL\ wrt.\ proofs, which, in other words, can be understood as soundness and completeness wrt.\ categorical models.


The following rule (\agdaref{Sound.agda\#L21}) is admissible for any $\mc{C}[\bullet]$ and $f : A \lra B$  where ${-}^* : \Tree \to \Fma$ is a function that replaces all commas by $\ot$ in a tree:
% (\agdaref{Sound.agda\#L13})
  \begin{equation}\label{lem:f2Tf}
    \infer[\mc{C}{[f]}]{(\mc{C}[A])^* \lra (\mc{C}[B])^*}{
    \deduce{A \lra B}{f}
  }
  \end{equation}


The sequent (\ref{eq:seqcalc}) and axiomatic (\ref{eq:axiomatic:calculus}) calculi are equivalent wrt.\ proofs, i.e.\ there exist two functions $\sound : T \vd C \to T^* \lra C$ (\agdaref{Sound.agda\#L63}) and $\complt : A \lra C \to A \vd C$ (\agdaref{Complt.agda\#L15}) such that 
  \begin{enumerate}
    ...
    \item for $f : T \vd C$, $\complt (\sound ~ f) \circeq \tl^* ~ f$ (\agdaref{CompltSound.agda\#L26}), and
    \item for $f : A \lra C$, $\sound (\complt ~ f) \doteq f$ (\agdaref{SoundComplt.agda\#L228}).
  \end{enumerate}
  $\tl^*$ is a derivable rule of multiple applications of $\tl$.

  The functions $\sound$ and $\complt$ are both constructed by induction on height of derivations.
  ...
  $}
  \]
  The first two properties are implemented as the following functions in Agda, respectively:\[
    \begin{array}{ll}
     \textit{1.} & \begin{array}{l}
      {\circeq}\sound{\doteq} : \forall \{T ~ C\} \to \{f ~ g : T \vd C\}  \to (eq : f \circeq g) \to \sound ~ f \doteq \sound ~ g
     \end{array}
     \\[2pt]
     \textit{2.} & \begin{array}{l}
      {\doteq}\complt{\circeq} : \forall \{A ~ B\} \to \{f ~ g : A \lra B\}  \to (eq : f \doteq g) \to \complt ~ f \circeq \complt ~ g
     \end{array}
    \end{array}
    \]
  They are both constructed by patter-matching on $eq$, the proof of equivalence of derivations. 
  The definition of ${\circeq}\sound{\doteq}$ relies on several properties of the admissible rule (\ref{lem:f2Tf}), including well-behaved wrt. $\comp$ (\agdaref{Sound.agda\#L29}), path concatenation (\agdaref{Sound.agda\#L37}), disjoint instantiation (\agdaref{Sound.agda\#L44}), and congruence wrt. $\doteq$ (\agdaref{Sound.agda\#L54}).
  On the other hand, the definition of ${\doteq}\complt{\circeq}$ heavily relies on the rules $\cut{\circeq}_1$ and $\cut{\circeq}_2$, i.e.\ the admissible cut rule respects $\circeq$ in each of its arguments. 
  
  The last two properties are implemented as the following, respectively:
  \[
  \begin{array}{ll}
    \textit{3.} & \begin{array}{l}
      \mf{compltsound} : \forall \{T ~ C\} \to (f : T \vd C) \to \complt (\sound ~ f) \circeq \tl^* f
      \end{array}
    \\[2pt]
    \textit{4.} & \begin{array}{l}
      \mf{soundcomplt} : \forall \{A ~ B\} \to (f : A \lra B) \to \sound(\complt ~ f) \doteq f
    \end{array}
  \end{array}
  \]
  These two properties are both defined by pattern-matching on $f$. 
  In the definition of $\mf{soundcomplt}$, we define a lemma $\mf{soundcut}$ (\agdaref{SoundComplt.agda\#L44}) which explicitly says that the admissible $\cut$ (\ref{eq:cut}) corresponds to $\comp$ in the axiomatic calculus (\ref{eq:axiomatic:calculus}).
