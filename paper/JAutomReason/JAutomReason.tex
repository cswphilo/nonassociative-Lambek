...
%  \multicolumn{2}{l}{\quad \mf{open} ~ \MBCC}\\
  ...
  \;\; \quad \mf{m} &: \{B ~ C : \Obj^\mb{C}\} \to \EqC{D}{(\mf{F}_0 ~ B \otz^\mb{D} \mf{F}_0 ~ C)}{(\mf{F}_0 ~ (B \otz^\mb{C} C))} \\[2pt]
  \;\; \quad \mf{m}^{-1} &: \{B ~ C : \Obj^\mb{C}\} \to \EqC{D}{(\mf{F}_0 ~(B \otz^\mb{C} C))}{(\mf{F}_0 ~B \otz^\mb{D} \mf{F}_0 ~C)} \\[2pt]
...
% We then show that $\FMBCC$ is the free magmatic biclosed category.
  Given a magmatic biclosed category $(\mb{D} , \ot' , \Rarr' , \Larr')$. For any function $F_{\mf{At}} : \mf{At} \to |\mb{D}|$, there exists a unique strict magmatic biclosed functor $F : \FMBCC \to \mb{D}$, i.e.\ $\FMBCC$ is the free magmatic biclosed category.
\\
