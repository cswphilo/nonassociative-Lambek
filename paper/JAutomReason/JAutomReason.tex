...
This paper is an extended version of \cite{VW2025Tableaux}. The whole Section \ref{sec:axiomatic} on the Hilbert-style axiomatic calculus for \NL, is new. It includes categorical models of \NL\ and equivalence between axiomatic and sequent calculi.
We have also added examples of the constructions of the $\mf{mip}$ and $\mf{mip}{\circeq}$ functions in Section \ref{sec:proof:rel:mip} to provide a better explanation of the idea.
...
The syntactic category $\FMBCC$ is moreover the \emph{free} magmatic biclosed category generated by the set $\At$.  This means that, given another magmatic biclosed category $(\mb{D} , \ot' , \Rarr' , \Larr', \piR', \piL')$ and a valuation function $F_{\mf{At}} : \mf{At} \to |\mb{D}|$ assigning objects of $\mb{D}$ to atomic formulae, there exists a unique strict magmatic biclosed functor $F : \FMBCC \to \mb{D}$ such that $F \circ \mf{at} = F_{\mf{At}}$, where $\mf{at} : \mf{At} \to \Fma$ is the inclusion of atomic formulae into the set of formulae.
...
% (\agdaref{Sound.agda\#L13})
  ...
  In Agda, properties $(i)$ and $(ii)$ are implemented as the following functions:
  \[
    \begin{array}{l}
  ...
  The construction of the function ${\circeq}\sound{\doteq}$ relies on several properties of the admissible rule $\mf{cxt}$ in (\ref{lem:f2Tf}), which are collected in Figure \ref{fig:cxt}.
  From top to bottom,  these properties state that:
  \begin{itemize}
    \item $\mf{cxt}$ preserves composition $\comp$ (\agdaref{Sound.agda\#L29});
    \item  $\mf{cxt}$ applied to a composite context results in two applications of $\mf{cxt}$ (\agdaref{Sound.agda\#L37}) (in the first derivation, the context $\mc{C}[\mc{C'}]$ is obtained by plugging the context $\mc{C'}$ in a hole of $\mc{C}$);
    \item two applications of $\mf{cxt}$ on disjiont parts of the context can be swapped (\agdaref{Sound.agda\#L44}).
  \end{itemize}
  Moreover, it is necessary to prove that $\mf{cxt}$ respects the congruence relation $\doteq$ (\agdaref{Sound.agda\#L54}).
  The consruction of the function ${\doteq}\complt{\circeq}$ heavily relies on  $\cut{\circeq}_1$ and $\cut{\circeq}_2$, i.e.\ the fact that the admissible cut rule respects $\circeq$ in each of its arguments.
...
\end{figure}
  ...
  In Agda, properties $(iii)$ and $(iv)$ are implemented as the following functions:
  \[
  \begin{array}{l}
  ...
  In the definition of $\mf{soundcomplt}$ we employ a lemma $\mf{soundcut}$ (\agdaref{SoundComplt.agda\#L44}), which states that the function $\sound$ sends composite (i.e. obtained via an application of the admissible $\cut$ rule) derivation in the sequent calculus to composite (i.e. obtained via an application of the primitive $\comp$ rule) derivation in the axiomatic calculus.
