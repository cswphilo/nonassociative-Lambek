...
So a magmatic category can be thought of as a monoidal category without unit object that is not required to have unitors and associators natural isomorphisms, as well as MacLane's triangle and pentagon coherence equations. The biclosed structure is added to capture the two implications of \NL.
...
Additionally, there is a family of functions $\pi_{\mf{R}}$, with inverses $\pi^{-1}_{\mf{R}}$, between $\Homs{B}{(A \Rarro C)}$ and $\Homs{(A \otz B)}{C}$, which underlie the natural isomorphism evidencing the adjunction $A \ot {-} \dashv A \Rarr {-}$.
There are analogous families of functions $\pi_{\mf{L}}$ and $\pi^{-1}_{\mf{L}}$ for the second adjunction in (\ref{eq:adj}) which we do not display here.
...
Let $(\mb{C} , \ot , \Rarr , \Larr)$ and $(\mb{D} , \ot' , \Rarr' , \Larr')$ be magmatic biclosed categories. A \emph{strong magmatic biclosed functor} $F : \mb{C} \to \mb{D}$ is a functor together with natural isomorphisms
\[
\begin{array}{c}
  m : FA \ot' FB \cong F (A \ot B)\\
  r : F (A \Rarr B) \cong FA \Rarr' FB
  \quad
  l : F(B \Larr A) \cong FB \Larr' FA
\end{array}
\]
preserving the families of functions $\piR$ and $\piL$, i.e.\ for any $f : A \ot B \to C$ in $\mb{C}$,
\[
r \circ F(\piR ~ f) = \piR (F f \circ m)
\quad
l \circ F(\piL ~ f) = \piL (F f \circ m).
\]
...
Below we show the constructors of actions on objects and morphisms, the natural isomorphism $m$ and preservation of $\piR$ and $\piRm$.
...
$}
\]
A strong magmatic biclosed functors is \emph{strict} when $m$, $r$, and $l$ are actual identities (\agdaref{FCatUniversal.agda\#L206}).
...
%  \multicolumn{2}{l}{\quad \mf{constructor} ~ \mf{intrp}{\circeq}} \\
  ...
  \;\; \quad eqg &: \subst ~(\lambda x.\ \sub{p}{(\eta ~ x)} \vd C)~eqD~g \circeq g' \\[2pt]
