...
Consider the case where $f = \Rl (f' , f'')$
\[
...
\] 
which tells us an equation $\mc{C'}[W , A \Rarr B] = \mc{C}[U]$.

In the Agda formalization, the equality appears as the type $\sub{p'}{W \cdast \eta (A \Rarr B)} \equiv \sub{p}{U}$ of the argument $eq$, where $p'$ is the path corresponds to the context $\mc{C'}$.
We apply $\subeq$ to the equality proof $eq$ to distinguish seven cases of the equation (\agdaref{Interpolation.agda\#L29}):
\begin{itemize}
  \item[--] $\oneeqtwo$, where $U$ is exactly equal to $W, A \Rarr B$;
  ...
  \item[--] $\oneLtwoR$ and $\oneRtwoL$, where $U$ are $W, A \Rarr B$ are disjoint in two different ways.
\end{itemize}
...
\end{array}
\]
The corresponding snippet in Agda is the following:
\[
\begin{array}{rl}
  \;\; \quad \dots &\vert ~ \onegtRtwo ~ (\bullet ~ \refl ~ \refl ~ \refl) = \\[2pt]
  \;\; \quad &\mf{let} ~ D ~ g' ~ h' = \mf{mip} ~ \bullet ~ W ~ f' ~ \refl \\[2pt]
  \;\; \quad & \quad \ E ~ g'' ~ h'' = \mf{mip} ~ p' ~ (\eta ~ B) ~ f' ~ \refl \\[2pt]
  \;\; \quad & \mf{in} ~ (D \Rarr E) ~ (\Rl ~ p' ~ h' ~h'') ~ (\Rr (\Rl ~ \bullet ~ g' ~ h''))
\end{array}
\]
...
\end{array}
\]
One example of the interesting cases is the parallel application of two instances of $\Rl$: 
\[
\scalebox{0.98}{$
\proofbox{
    ...
    $}
\]
which tells us an equation $\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']] = \mc{C}[U]$ that turns out to be the type $\sub{p'}{(\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B'])} \equiv \sub{p}{U}$ in Agda.
In this case, we apply the lemma $\subeq$ to the equality proof $eq$ to distinguish seven cases of the equation.
However, in some cases, e.g. $\mc{C'}[\mc{C}_1[V , A \Rarr B] , \mc{C}_2[W , A' \Rarr B']]$ contains $U$ in its left or right subtree, we have to apply the lemma $\subeq$ to the new equality proof $eq' : \sub{p_1}{\mc{C}_1[V , A \Rarr B]}$ (or $\sub{p_2}{\mc{C}_2[W , A' \Rarr B']}$) to further distinguish another seven cases.
