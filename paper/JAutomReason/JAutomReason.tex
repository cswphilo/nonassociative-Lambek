...
There exist various equivalent presentations of the axiomatic calculus for \NL\ in the literature. 
...
Our formulation in (\ref{eq:axiomatic:calculus}) is closer to the one proposed by Moortgat and Oehrle \cite{moot:categorial:2012}, which contains both the monotonicity and adjunction rules, but we add the transitivity rule (morphism composition) to make it a direct characterization of the categorical models.
...
\begin{equation}\label{eq:constructor:axiomatic:calc}
  ...
  \;\; \quad \circ &: \{A ~ B ~ C : \Fma \} \to (f : A \lra B) \to (g : B \lra C) \to A \lra C \\
  \;\; \quad \Rarr &: \{A ~ B ~ C ~ D : \Fma \} \to (f : B \lra A) \to (g : C \lra D) \\
  \;\; \quad &\hspace*{0.5cm}\to A \Rarr C \lra B \Rarr D \\
  \;\; \quad \pi_{\Rarr} &: \{A ~ B ~ C : \Fma \} \to (f : A \ot B \lra C) \to B \lra A \Rarr C \\
  \;\; \quad \pi^{-1}_{\Rarr} &: \{A ~ B ~ C : \Fma \} \to (f : B \lra A \Rarr C) \to A \ot B \lra C
\end{array}
\end{equation}

To interpret derivations as morphisms, we define an equivalence relation $\doteq$ on derivations (Figures \ref{fig:cat:eq},\ref{fig:eq:Rarr} and \ref{fig:eq:Larr}) that respects the definition of a category and moreover the magmatic biclosed structure.
In Figure \ref{fig:cat:eq}, the first two lines of equations characterize the left and right unitality of identity morphisms and associativity of composition, respectively.
The last two lines are the equations that characterize the functoriality of $\ot$.

In Figure \ref{fig:eq:Rarr}, the first two equations are for the functoriality of $\Rarr$. The middle three equations correspond to naturality of the three arguments of the adjunction, respectively. The last two equations characterize that $\pi_{\Rarr}$ and $\pi^{-1}_{\Rarr}$ are each other's inverses.
Figure \ref{fig:eq:Larr} contains the similar equations as those in Figure \ref{fig:eq:Rarr} for $\Larr$.

The equivalence relation $\doteq$ is implemented as an inductive family indexed over pairs of derivations (\agdaref{FCat.agda\#L34}), which is similar to the formalization of $\circeq$ in Section \ref{sec:equiv:der}.
\begin{figure}[t]
  % \vspace{0.5\textwidth}
%\begin{landscape}
\[
...
\begin{figure}[t]
  \begin{displaymath}
    ...
    \end{array}$}
  \end{displaymath}
  \caption{Functoriality of \(\Rarr \) and naturality of the adjunction}
  \label{fig:eq:Rarr}
\end{figure}
...
\begin{figure}[t]
  \begin{displaymath}
    ...
    \end{array}$}
  \end{displaymath}
  \caption{Functoriality of \(\Larr \) and naturality of the adjunction}
  \label{fig:eq:Larr}
\end{figure}
\subsection{Free Magmatic Biclosed Category}
Let $\FMBCC$ be the category generated by (\ref{eq:axiomatic:calculus}) with the equivalence relation $\doteq$ (\agdaref{FCatUniversal.agda\#L249}) over the set $\At$.
It has formulae ($\Fma$) as objects; connectives $\ot$, $\Rarr$ and $\Larr$ as the functors. The set of maps between two objects $A$ and $B$ are given by the set of derivations of the sequent $A \lra B$ quotiented by $\doteq$. The identity maps on objects are the equivalence classes of $\id$ while composition is given by $\comp$.

% We then show that $\FMBCC$ is the free magmatic biclosed category.
  Given a magmatic biclosed category $(\mb{D} , \ot' , \Rarr' , \Larr')$. For any function $F_{\mf{At}} : \mf{At} \to |\mb{D}|$, there exists a unique strict magmatic biclosed functor $F : \FMBCC \to \mb{D}$, i.e.\ $\FMBCC$ is the free magmatic biclosed category generated over the set $\At$.
\\
\udl{Existence (\agdaref{FCatUniversal.agda\#L310})}
\\
  ...
  \end{array}
  \]
The action $F_1$ on morphisms is defined inductively on the height of derivations which maps each morphism $\FMBCC(A , B)$ to $\mb{D} (F_0 A , F_0 B)$ by replacing $\ot$ to $\ot'$, $\Rarr$ to $\Rarr'$ and $\Larr$ to $\Larr'$, respectively.

Given $F_0$ and $F_1$, we define a functor $F : \FMBCC \to \mb{D}$ that is strict and preserves the magmatic biclosed structure.
\\
\udl{Uniqueness (\agdaref{FCatUniversal.agda\#L411})}
\\
Consider another magmatic biclosed functor $G : \FMBCC \to \mb{D}$ that the action on objects agrees with the function $F_{\mf{At}}$. We construct a natural isomorphism $t : F \to G$ by induction on formulae and derivations. Therefore, if $G$ is strict, then $F$ and $G$ agree on all objects and morphisms in $\FMBCC$.
