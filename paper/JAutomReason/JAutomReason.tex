...
\newcommand{\agdahref}{562cf43c4c496e18e3e6b1ff1d2d9d659500db63}
...
\section{Axiomatic Presentation of Nonassociative Lambek Calculus}
The axiomatic (Hilbert-style) calculus of \NL\ was first proposed by Lambek \cite{lambek1988} for linguistic analysis.
Do{\v s}en \cite{Dosen1988} and Moortgat and Oehrle (\cheng{The book by Moot and Retore mentioned this paper but I can't find it. I am not sure if we should cite the original paper or just their book.}) also proposed different axiomatic formulations with applications in soundness and completeness with respect to ternary relational models and complexity of \NL, respectively.

Our formulation is similar to Lambek's, but we add the the monotonicity (functorial) rules of the logical connectives to make it a direct characterization of its categorical model.
By a direct characterization of its categorical model, we mean that the axiomatic calculus forms a category where we interpret objects as formulae, derivations as morphisms and composition as the rule $\comp$.
...
\begin{equation}\label{eq:constructor:axiomatic:calc}
  ...
  \;\; \quad \Rarr &: \{A ~ B ~ C ~ D : \Fma \} \to (f : B \lra A) \to (g : C \lra D) \to A \Rarr C \lra B \Rarr D \\
...
The first group defines objects and actions on objects:
 \[
\begin{array}{ll}
  \multicolumn{2}{l}{\record \:\: \MBCC : \Set_{\mf{1}} \:\: \where} \\
  \multicolumn{2}{l}{\quad \field} \\
  \;\; \quad \Obj &: \Set \\[0.5pt]
  \;\; \quad \otz &: \Obj \to \Obj \to \Obj \\[0.5pt]
  \;\; \quad \Rarrz &: \Obj \to \Obj \to \Obj \\[0.5pt]
  \;\; \quad \Larrz &: \Obj \to \Obj \to \Obj
\end{array}
\]
The second group defines the morphisms and actions on morphism.
...
\scalebox{0.98}{$
\begin{array}{ll}
  \;\; \quad \Hom &: \Obj \to \Obj \to \Set \\[0.5pt]
  \;\; \quad \mf{Id} &: \{B : \Obj\} \to \Homs{B}{B} \\[0.5pt]
  \;\; \quad \mf{Comp} &: \{D ~ B ~ C : \Obj\} \to \Homs{B}{C} \to \Homs{D}{B} \to \Homs{D}{C} \\[0.5pt]
  \;\; \quad \Rarro &: \{A ~ B ~ C ~ D : \Obj\} \to \Homs{B}{A} \to \Homs{C}{D} \to \Homs{(\Rarrz A ~ C)}{(\Rarrz B ~ D)} \\[0.5pt]
  \;\; \quad \pi_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{(\otz ~ A ~ B)}{C} \to \Homs{B}{(\Rarrz A ~ C)} \\[0.5pt]
...
Here we show the constructors of naturality of $\pi_{\mf{R}}$ and $\pi_{\mf{R}}$ and $\pi^{-1}_{\mf{R}}$ are each other's inverses.
\[
\begin{array}{ll}
  \;\; \quad \mf{Eq} &: \{A ~ B\} \to \Homs{A}{B} \to \Homs{A}{B} \to \Set \\[0.5pt]
  \;\; \quad \piR\piRm &: \{A ~ B ~ C\} ~ \{f : \Homs{(\otz ~ A ~ B)}{C}\} \to \Eq{(\piRm (\piR ~ f))}{f}\\[0.5pt]
  \;\; \quad \mf{Id} &: \{B : \Obj\} \to \Homs{B}{B} \\[0.5pt]
  \;\; \quad \mf{Comp} &: \{D ~ B ~ C : \Obj\} \to \Homs{B}{C} \to \Homs{D}{B} \to \Homs{D}{C} \\[0.5pt]
  \;\; \quad \Rarro &: \{A ~ B ~ C ~ D : \Obj\} \to \Homs{B}{A} \to \Homs{C}{D} \to \Homs{(\Rarrz A ~ C)}{(\Rarrz B ~ D)} \\[0.5pt]
  \;\; \quad \pi_{\mf{R}} &: \{A ~ B ~ C : \Obj\} \to \Homs{(\otz ~ A ~ B)}{C} \to \Homs{B}{(\Rarrz A ~ C)} \\[0.5pt]
...
$\FMBCC$ is the category generated by (\ref{eq:axiomatic:calculus}) with the equivalence relation $\doteq$.
...
We then show that $\FMBCC$ is the free magmatic biclosed category, i.e.\ it captures all canonical morphisms and equations of magmatic biclosed categories.


\begin{theorem}
  Given a magmatic biclosed category $(\mb{D} , \ot' , \Rarr' , \Larr')$. For any function $F_{\mf{At}} : \mf{At} \to |\mb{D}|$, there exists a unique strict magmatic biclosed functor $F : \FMBCC \to \mb{D}$.
\end{theorem}
\vspace*{-0.5cm}
\begin{proof}
~
...
\\
  ...
  \end{array}
  \]
The action $F_1$ on morphisms is defined inductively on the height of derivations which maps each morphism $\FMBCC(A , B)$ to $\mb{D} (F_0 , F_0 B)$ by replacing $\ot$ to $\ot'$, $\Rarr$ to $\Rarr'$ and $\Larr$ to $\Larr'$, respectively.

Given $F_0$ and $F_1$, we define a functor $F : \FMBCC \to \mb{D}$ that is strict and preserves the magmatic biclosed structure.

\udl{Uniqueness}
\\
Consider another strict magmatic biclosed functor $G : \FMBCC \to \mb{D}$ that the action on objects agrees with the function $F_{\mf{At}}$. We can verify that $F$ and $G$ agree on all objects and morphisms in $\FMBCC$ by induction on formulae and derivations.
\end{proof}
\cheng{maybe some text about why this shows $\FMBCC$ is the free category.}
\begin{enumerate}
  \item first the Hilbert-style calculus and equations
  \item categorical semantics of the calculus
  \item soundness and completeness wrt. proofs where the admissible cut plays an important role and the unitality, associativity and commutativity of cut.
  \item now I have the calculus and equations, so the next step is to write down the definition of magmatic biclosed categories.
  \item then we can move to the free construction, maybe we can first write down the proof-relevant equivalence between the axiomatic and sequent calculi
  \item then we can show the axiomatic calculus with equations is the free magmatic biclosed category
  \item then by the proof-relevant equivalence, the sequent calculus with equations is also the free magmatic biclosed category up to isomorphism.
\end{enumerate}
...
In this section we show the equivalence between the sequent (\ref{eq:seqcalc}) and axiomatic (\ref{eq:axiomatic:calculus}) calculi of \NL\ with respect to proofs, which, in other words, can be understood as soundness and completeness with respect to categorical models.

\begin{definition}
  We define a function ${-}^* : \Fma \to \Tree$ that replaces all commas to $\ot$ in a tree.
\end{definition}

\begin{theorem}\label{thm:equiv:calculi}
  The sequent (\ref{eq:seqcalc}) and axiomatic (\ref{eq:axiomatic:calculus}) calculi are equivalent with respect to proofs, i.e.\ there exist two functions $\sound : T \vd C \to T^* \lra C$ and $\complt : A \lra C \to A \vd C$ such that 
  \begin{enumerate}
    \item for $f , g : T \vd C$, if $f \circeq g$, then $\sound f \doteq \sound g$,
    \item for $f , g : A \lra C$, if $f \doteq g$, then $\complt f \doteq \complt g$,
    \item for $f : T \vd C$, $\complt (\sound f) \circeq f$, and
    \item for $f : A \lra C$, $\sound (\complt f) \doteq f$.
  \end{enumerate}
\end{theorem}
\begin{proof}
  The functions $\sound$ and $\complt$ are both constructed by induction on height of derivations.
  The first two properties are proved by induction on the equivalence of derivations $\circeq$ and $\doteq$ respectively, while the last two are proved by induction on height of derivations.
\end{proof}
