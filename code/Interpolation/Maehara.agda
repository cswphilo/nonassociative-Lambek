...
module Interpolation.Maehara where
...
-- Another version of mip, doing the same exact same thing, but with
-- different "with subeq", matching the one uses in cut admissibility
...
mip' p _ (⇐R f) refl = 
  let intrp D g h = mip' (p ◂ _) _ f refl
   in intrp D (⇐R g) h
mip' p U (⇒L p' f f') eq with subeq _ _ p' p eq
... | 1>L2 (gt q refl refl refl) = 
  let intrp D g h = mip' p _ f' refl
  in intrp D g (⇒L (q ◂ _) f h)
... | 1>R2 (gt q refl refl refl) = 
...
... | 2>L1 (gt q refl refl refl) =
  let intrp D g h = mip' q _ f refl
   in intrp D (⇒L p' g f') h
... | 2>R1 (gt ∙ refl refl refl) =
  let intrp D k l = mip' ∙ _ f refl
      intrp E g h = mip' p' _ f' refl
  in intrp (D ⇒ E) (⇒L p' l g) (⇒R (⇒L ∙ k h))
mip' p _ (⇐L p' f f') eq with subeq _ _ p' p eq
... | 1>L2 (gt q refl refl refl) =
  let intrp D g h = mip' p _ f' refl
   in intrp D g (⇐L (q ◂ _) f h)
... | 1>R2 (gt q refl refl refl) =
...
mip' (T ▸ p) U (⊗R f f') refl =
  let intrp D g h = mip' p _ f' refl
  in intrp D (⊗R f g) h
mip' p U (⊗L p' f) eq with subeq _ _ p' p eq
... | 1≡2 (same refl refl refl) =
  let intrp D g h = mip' p _ f refl
   in intrp D g (⊗L ∙ h)
... | 1>L2 (gt q refl refl refl) = 
